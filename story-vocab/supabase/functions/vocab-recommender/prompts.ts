...
5. **預測性推薦**（重要）：
   - 基於故事主題和已有上下文，預測「接下來」可能的發展方向
   - 推薦能幫助故事繼續的詞語，而非摘取已有句子中的詞
   - 面向故事的未來走向，而非回顧過去
   - 例：故事提到「神秘古書」→ 推薦「秘密、探索、魔法」而非「古書、提到」
...
請推薦5個詞，確保：
1. **預測故事發展**：推薦能幫助故事「接下來」發展的詞，而非摘取已有句子的詞
2. **語境適配**：詞語應該適合故事的主題和氛圍
3. 難度呈梯度分布（例如：L2, L3, L3, L4, L4）
4. 至少${roundNumber <= 3 ? '3' : '4'}個是新詞
5. **所有推薦的詞都不能與本次遊戲已推薦過的詞重複**
6. **優先參考用戶的 current_level 和最近選詞難度，而不是年級**
7. **難度值只能是 1, 2, 3, 4, 5（整數），不可超過 5**
