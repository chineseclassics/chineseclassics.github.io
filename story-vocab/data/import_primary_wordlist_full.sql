-- =====================================================
-- 小學中文字詞表 (2025) - 完整 SQL 導入腳本
-- 自動生成於：2025-10-17
-- 總詞語數：1308 個
-- =====================================================

BEGIN;

-- ========================================
-- 1. 創建詞表記錄
-- ========================================

INSERT INTO wordlists (
  name,
  code,
  type,
  owner_id,
  hierarchy_config,
  description,
  total_words,
  is_public
) VALUES (
  '小學中文字詞表（2025）',
  'primary_chinese_2025',
  'system',
  NULL,
  '{"level_2_label": "單元", "level_3_label": "課文"}'::jsonb,
  '香港小學中文課本字詞表（一年級至五年級），包含 1308 個核心詞語，按單元和課文組織。',
  1308,
  true
)
ON CONFLICT (code) DO UPDATE
SET 
  name = EXCLUDED.name,
  description = EXCLUDED.description,
  total_words = EXCLUDED.total_words,
  updated_at = NOW();

-- ========================================
-- 2. 創建層級標籤
-- ========================================

-- 第二層級標籤（單元）
INSERT INTO wordlist_tags (wordlist_id, tag_level, tag_code, tag_display_name, sort_order)
SELECT 
  (SELECT id FROM wordlists WHERE code = 'primary_chinese_2025'),
  2,
  tag_name,
  tag_name,
  ROW_NUMBER() OVER (ORDER BY tag_name)
FROM (VALUES
  ('一上單元一'),
  ('一上單元二'),
  ('一下單元一'),
  ('一下單元二'),
  ('三上單元一'),
  ('三上單元二'),
  ('三下單元一'),
  ('三下單元二'),
  ('二上單元一'),
  ('二上單元二'),
  ('二下單元一'),
  ('二下單元二'),
  ('五上單元一'),
  ('五上單元二'),
  ('五下單元一'),
  ('五下單元二'),
  ('四上單元一'),
  ('四上單元二'),
  ('四下單元一'),
  ('四下單元二')
) AS tags(tag_name)
ON CONFLICT (wordlist_id, tag_code) DO NOTHING;

-- 第三層級標籤（課文）
INSERT INTO wordlist_tags (wordlist_id, tag_level, tag_code, tag_display_name, sort_order)
SELECT 
  (SELECT id FROM wordlists WHERE code = 'primary_chinese_2025'),
  3,
  tag_name,
  tag_name,
  ROW_NUMBER() OVER (ORDER BY tag_name)
FROM (VALUES
  ('一分鐘'),
  ('一束鮮艷的花'),
  ('上學歌'),
  ('上默書課'),
  ('中彩那天'),
  ('什麼比獵豹的速度更快'),
  ('保羅的自行車'),
  ('剃頭大師'),
  ('動物王國開大會'),
  ('南極的主人'),
  ('參觀青馬大橋'),
  ('在海裏'),
  ('多彩的賀卡'),
  ('夜間飛行的祕密'),
  ('大熊貓'),
  ('大自然的聲音'),
  ('大還是小'),
  ('太陽'),
  ('奇妙的漢字'),
  ('奶奶笑了'),
  ('孫悟空，變變變！'),
  ('富饒的西沙羣島'),
  ('將相和'),
  ('小壁虎借尾巴'),
  ('小心眼'),
  ('小書包'),
  ('小木船'),
  ('小雨傘'),
  ('小馬過河'),
  ('廉頗和藺相如'),
  ('愛的紙條'),
  ('慧娟怎樣長大'),
  ('我愛故鄉的楊梅'),
  ('我的名字叫做貓'),
  ('我的球迷哥哥'),
  ('扁鵲治病（白話文）'),
  ('打掃房子'),
  ('找春天'),
  ('把太陽送給媽媽'),
  ('拔牙'),
  ('掌聲'),
  ('文具的家'),
  ('早操'),
  ('春天'),
  ('晏子使楚'),
  ('曹沖稱象'),
  ('會捕食的植物'),
  ('東方之珠'),
  ('桂林山水'),
  ('歡歡喜喜包餃子'),
  ('沙灘上的腳印'),
  ('清澈的湖水'),
  ('溜冰場上'),
  ('火星——地球的「孿生兄弟」'),
  ('火燒雲'),
  ('爸爸的花兒落了（節選）'),
  ('狐狸和烏鴉'),
  ('王戎智捉人販子'),
  ('玲玲的畫'),
  ('田忌賽馬'),
  ('白雲'),
  ('秋天的雨'),
  ('紀昌學射'),
  ('紙的發明'),
  ('綠樹枱燈'),
  ('美味的粽子'),
  ('美麗的中華白海豚'),
  ('美麗的小興安嶺'),
  ('美麗的香山'),
  ('膽小的爸爸'),
  ('花之路'),
  ('荷葉圓圓'),
  ('菜市場'),
  ('蜘蛛開店'),
  ('西湖風光'),
  ('觀潮'),
  ('諸葛亮巧佈空城計'),
  ('買食物'),
  ('遊海洋公園'),
  ('遊迪士尼樂園'),
  ('遙控車壞了'),
  ('鄉下人家'),
  ('釣魚的啟示'),
  ('雨點兒'),
  ('雪地裏的小畫家'),
  ('霧鎖香江'),
  ('露珠'),
  ('風在哪兒'),
  ('香噴噴的夢'),
  ('騾子和冰'),
  ('鯨'),
  ('黃山奇石')
) AS tags(tag_name)
ON CONFLICT (wordlist_id, tag_code) DO NOTHING;

-- ========================================
-- 3. 導入詞彙數據
-- ========================================

-- 批量插入所有詞語
INSERT INTO wordlist_vocabulary (wordlist_id, word, level_2_tag, level_3_tag)
SELECT 
  (SELECT id FROM wordlists WHERE code = 'primary_chinese_2025'),
  word,
  level_2_tag,
  level_3_tag
FROM (VALUES
  ('上學', '一上單元一', '上學歌'),
  ('太陽', '一上單元一', '上學歌'),
  ('小朋友', '一上單元一', '上學歌'),
  ('為甚麼', '一上單元一', '上學歌'),
  ('書包', '一上單元一', '上學歌'),
  ('學校', '一上單元一', '上學歌'),
  ('老師', '一上單元一', '上學歌'),
  ('同學', '一上單元一', '上學歌'),
  ('外婆', '一上單元一', '小書包'),
  ('身上', '一上單元一', '小書包'),
  ('神氣', '一上單元一', '小書包'),
  ('書本', '一上單元一', '小書包'),
  ('整齊', '一上單元一', '小書包'),
  ('漂亮', '一上單元一', '小書包'),
  ('文具', '一上單元一', '小書包'),
  ('作用', '一上單元一', '小書包'),
  ('天天', '一上單元一', '小書包'),
  ('早操', '一上單元一', '早操'),
  ('樹葉', '一上單元一', '早操'),
  ('樹枝', '一上單元一', '早操'),
  ('蜜蜂', '一上單元一', '早操'),
  ('花朵', '一上單元一', '早操'),
  ('陽光', '一上單元一', '早操'),
  ('奶奶', '一上單元一', '奶奶笑了'),
  ('放學', '一上單元一', '奶奶笑了'),
  ('看見', '一上單元一', '奶奶笑了'),
  ('生病', '一上單元一', '奶奶笑了'),
  ('照顧', '一上單元一', '奶奶笑了'),
  ('開水', '一上單元一', '奶奶笑了'),
  ('蘋果', '一上單元一', '奶奶笑了'),
  ('故事', '一上單元一', '奶奶笑了'),
  ('高興', '一上單元一', '奶奶笑了'),
  ('有時候', '一上單元一', '大還是小'),
  ('覺得', '一上單元一', '大還是小'),
  ('自己', '一上單元一', '大還是小'),
  ('衣服', '一上單元一', '大還是小'),
  ('鞋帶', '一上單元一', '大還是小'),
  ('門鈴', '一上單元一', '大還是小'),
  ('聽到', '一上單元一', '大還是小'),
  ('雷聲', '一上單元一', '大還是小'),
  ('希望', '一上單元一', '大還是小'),
  ('長大', '一上單元一', '大還是小'),
  ('市場', '一上單元二', '菜市場'),
  ('今天', '一上單元二', '菜市場'),
  ('跟着', '一上單元二', '菜市場'),
  ('黃色', '一上單元二', '菜市場'),
  ('香蕉', '一上單元二', '菜市場'),
  ('白菜', '一上單元二', '菜市場'),
  ('新鮮', '一上單元二', '菜市場'),
  ('番茄', '一上單元二', '菜市場'),
  ('營養', '一上單元二', '菜市場'),
  ('回家', '一上單元二', '菜市場'),
  ('下雨', '一上單元二', '把太陽送給媽媽'),
  ('沒有', '一上單元二', '把太陽送給媽媽'),
  ('眼睛', '一上單元二', '把太陽送給媽媽'),
  ('着急', '一上單元二', '把太陽送給媽媽'),
  ('許多', '一上單元二', '把太陽送給媽媽'),
  ('那裏', '一上單元二', '把太陽送給媽媽'),
  ('擋雨', '一上單元二', '把太陽送給媽媽'),
  ('抬頭', '一上單元二', '把太陽送給媽媽'),
  ('回來', '一上單元二', '把太陽送給媽媽'),
  ('金色', '一上單元二', '把太陽送給媽媽'),
  ('一直', '一上單元二', '把太陽送給媽媽'),
  ('膽小', '一上單元二', '膽小的爸爸'),
  ('這樣', '一上單元二', '膽小的爸爸'),
  ('回答', '一上單元二', '膽小的爸爸'),
  ('每次', '一上單元二', '膽小的爸爸'),
  ('馬路', '一上單元二', '膽小的爸爸'),
  ('總是', '一上單元二', '膽小的爸爸'),
  ('緊緊', '一上單元二', '膽小的爸爸'),
  ('抓住', '一上單元二', '膽小的爸爸'),
  ('穿過', '一上單元二', '膽小的爸爸'),
  ('放開', '一上單元二', '膽小的爸爸'),
  ('沙灘', '一上單元二', '沙灘上的腳印'),
  ('腳印', '一上單元二', '沙灘上的腳印'),
  ('海浪', '一上單元二', '沙灘上的腳印'),
  ('歡快', '一上單元二', '沙灘上的腳印'),
  ('調皮', '一上單元二', '沙灘上的腳印'),
  ('打掃', '一上單元二', '打掃房子'),
  ('房子', '一上單元二', '打掃房子'),
  ('新年', '一上單元二', '打掃房子'),
  ('我們', '一上單元二', '打掃房子'),
  ('一起', '一上單元二', '打掃房子'),
  ('窗戶', '一上單元二', '打掃房子'),
  ('桌子', '一上單元二', '打掃房子'),
  ('地板', '一上單元二', '打掃房子'),
  ('收拾', '一上單元二', '打掃房子'),
  ('乾乾淨淨', '一上單元二', '打掃房子'),
  ('弟弟', '一上單元二', '打掃房子'),
  ('現在', '一上單元二', '打掃房子'),
  ('明亮', '一上單元二', '打掃房子'),
  ('雨傘', '一下單元一', '小雨傘'),
  ('草地', '一下單元一', '小雨傘'),
  ('蝴蝶', '一下單元一', '小雨傘'),
  ('池塘', '一下單元一', '小雨傘'),
  ('荷葉', '一下單元一', '小雨傘'),
  ('青蛙', '一下單元一', '小雨傘'),
  ('樹林', '一下單元一', '小雨傘'),
  ('蘑菇', '一下單元一', '小雨傘'),
  ('螞蟻', '一下單元一', '小雨傘'),
  ('大家', '一下單元一', '小雨傘'),
  ('數不清', '一下單元一', '雨點兒'),
  ('哪裏', '一下單元一', '雨點兒'),
  ('回答', '一下單元一', '雨點兒'),
  ('地方', '一下單元一', '雨點兒'),
  ('不久', '一下單元一', '雨點兒'),
  ('搖籃', '一下單元一', '荷葉圓圓'),
  ('亮晶晶', '一下單元一', '荷葉圓圓'),
  ('蜻蜓', '一下單元一', '荷葉圓圓'),
  ('展開', '一下單元一', '荷葉圓圓'),
  ('翅膀', '一下單元一', '荷葉圓圓'),
  ('青蛙', '一下單元一', '荷葉圓圓'),
  ('舞台', '一下單元一', '荷葉圓圓'),
  ('歌唱', '一下單元一', '荷葉圓圓'),
  ('涼帽', '一下單元一', '荷葉圓圓'),
  ('笑嘻嘻', '一下單元一', '荷葉圓圓'),
  ('綠油油', '一下單元一', '荷葉圓圓'),
  ('臉蛋', '一下單元一', '荷葉圓圓'),
  ('柳樹', '一下單元一', '春天'),
  ('說話', '一下單元一', '春天'),
  ('洗澡', '一下單元一', '春天'),
  ('春風', '一下單元一', '春天'),
  ('梳頭', '一下單元一', '春天'),
  ('捉迷藏', '一下單元一', '春天'),
  ('旅遊', '一下單元一', '春天'),
  ('泥土', '一下單元一', '春天'),
  ('種子', '一下單元一', '春天'),
  ('白雲', '一下單元一', '白雲'),
  ('孩子', '一下單元一', '白雲'),
  ('學習', '一下單元一', '白雲'),
  ('常常', '一下單元一', '白雲'),
  ('模仿', '一下單元一', '白雲'),
  ('飛跑', '一下單元一', '白雲'),
  ('山腰', '一下單元一', '白雲'),
  ('追逐', '一下單元一', '白雲'),
  ('玩耍', '一下單元一', '白雲'),
  ('一會兒', '一下單元一', '白雲'),
  ('魔術師', '一下單元一', '白雲'),
  ('天空', '一下單元一', '白雲'),
  ('千變萬化', '一下單元一', '白雲'),
  ('畫家', '一下單元二', '雪地裏的小畫家'),
  ('下雪', '一下單元二', '雪地裏的小畫家'),
  ('小雞', '一下單元二', '雪地裏的小畫家'),
  ('竹葉', '一下單元二', '雪地裏的小畫家'),
  ('小狗', '一下單元二', '雪地裏的小畫家'),
  ('梅花', '一下單元二', '雪地裏的小畫家'),
  ('小鴨', '一下單元二', '雪地裏的小畫家'),
  ('楓葉', '一下單元二', '雪地裏的小畫家'),
  ('小馬', '一下單元二', '雪地裏的小畫家'),
  ('月牙', '一下單元二', '雪地裏的小畫家'),
  ('顔料', '一下單元二', '雪地裏的小畫家'),
  ('參加', '一下單元二', '雪地裏的小畫家'),
  ('睡着', '一下單元二', '雪地裏的小畫家'),
  ('海豚', '一下單元二', '美麗的中華白海豚'),
  ('可愛', '一下單元二', '美麗的中華白海豚'),
  ('娃娃', '一下單元二', '美麗的中華白海豚'),
  ('波浪', '一下單元二', '美麗的中華白海豚'),
  ('小船', '一下單元二', '美麗的中華白海豚'),
  ('好像', '一下單元二', '美麗的中華白海豚'),
  ('妹妹', '一下單元二', '美麗的中華白海豚'),
  ('游泳', '一下單元二', '美麗的中華白海豚'),
  ('運動', '一下單元二', '美麗的中華白海豚'),
  ('美麗', '一下單元二', '美麗的中華白海豚'),
  ('彩虹', '一下單元二', '美麗的中華白海豚'),
  ('珍奇', '一下單元二', '大熊貓'),
  ('動物', '一下單元二', '大熊貓'),
  ('可愛', '一下單元二', '大熊貓'),
  ('胖乎乎', '一下單元二', '大熊貓'),
  ('毛茸茸', '一下單元二', '大熊貓'),
  ('黑眼圈', '一下單元二', '大熊貓'),
  ('活潑', '一下單元二', '大熊貓'),
  ('活動', '一下單元二', '大熊貓'),
  ('喜歡', '一下單元二', '大熊貓'),
  ('新鮮', '一下單元二', '大熊貓'),
  ('夏天', '一下單元二', '南極的主人'),
  ('一搖一擺', '一下單元二', '南極的主人'),
  ('十足', '一下單元二', '南極的主人'),
  ('靠近', '一下單元二', '南極的主人'),
  ('怎麼辦', '一下單元二', '南極的主人'),
  ('東張西望', '一下單元二', '南極的主人'),
  ('交頭接耳', '一下單元二', '南極的主人'),
  ('成群結隊', '一下單元二', '南極的主人'),
  ('生氣', '一下單元二', '南極的主人'),
  ('主人', '一下單元二', '南極的主人'),
  ('東西', '一下單元二', '南極的主人'),
  ('壁虎', '一下單元二', '小壁虎借尾巴'),
  ('尾巴', '一下單元二', '小壁虎借尾巴'),
  ('蚊子', '一下單元二', '小壁虎借尾巴'),
  ('咬住', '一下單元二', '小壁虎借尾巴'),
  ('逃走', '一下單元二', '小壁虎借尾巴'),
  ('難看', '一下單元二', '小壁虎借尾巴'),
  ('不行', '一下單元二', '小壁虎借尾巴'),
  ('河邊', '一下單元二', '小壁虎借尾巴'),
  ('燕子', '一下單元二', '小壁虎借尾巴'),
  ('阿姨', '一下單元二', '小壁虎借尾巴'),
  ('掌握', '一下單元二', '小壁虎借尾巴'),
  ('方向', '一下單元二', '小壁虎借尾巴'),
  ('難過', '一下單元二', '小壁虎借尾巴'),
  ('告訴', '一下單元二', '小壁虎借尾巴'),
  ('轉身', '一下單元二', '小壁虎借尾巴'),
  ('高興', '一下單元二', '小壁虎借尾巴'),
  ('鉛筆', '二上單元一', '文具的家'),
  ('橡皮擦', '二上單元一', '文具的家'),
  ('當然', '二上單元一', '文具的家'),
  ('所以', '二上單元一', '文具的家'),
  ('每天', '二上單元一', '文具的家'),
  ('趕緊', '二上單元一', '文具的家'),
  ('怎麼', '二上單元一', '文具的家'),
  ('平安', '二上單元一', '文具的家'),
  ('文具盒', '二上單元一', '文具的家'),
  ('從此', '二上單元一', '文具的家'),
  ('仔細', '二上單元一', '文具的家'),
  ('檢查', '二上單元一', '文具的家'),
  ('尺子', '二上單元一', '文具的家'),
  ('所有', '二上單元一', '文具的家'),
  ('助手', '二上單元一', '文具的家'),
  ('已經', '二上單元一', '文具的家'),
  ('鬧鐘', '二上單元一', '一分鐘'),
  ('哈欠', '二上單元一', '一分鐘'),
  ('心想', '二上單元一', '一分鐘'),
  ('遲到', '二上單元一', '一分鐘'),
  ('起來', '二上單元一', '一分鐘'),
  ('過去', '二上單元一', '一分鐘'),
  ('公共汽車', '二上單元一', '一分鐘'),
  ('影子', '二上單元一', '一分鐘'),
  ('決定', '二上單元一', '一分鐘'),
  ('上課', '二上單元一', '一分鐘'),
  ('座位', '二上單元一', '一分鐘'),
  ('手錶', '二上單元一', '一分鐘'),
  ('非常', '二上單元一', '一分鐘'),
  ('後悔', '二上單元一', '一分鐘'),
  ('得意', '二上單元一', '玲玲的畫'),
  ('端詳', '二上單元一', '玲玲的畫'),
  ('評獎', '二上單元一', '玲玲的畫'),
  ('時間', '二上單元一', '玲玲的畫'),
  ('收拾', '二上單元一', '玲玲的畫'),
  ('傷心', '二上單元一', '玲玲的畫'),
  ('報紙', '二上單元一', '玲玲的畫'),
  ('來不及', '二上單元一', '玲玲的畫'),
  ('懶洋洋', '二上單元一', '玲玲的畫'),
  ('滿意', '二上單元一', '玲玲的畫'),
  ('想像', '二上單元一', '玲玲的畫'),
  ('動腦筋', '二上單元一', '玲玲的畫'),
  ('變成', '二上單元一', '玲玲的畫'),
  ('背後', '二上單元一', '小心眼'),
  ('說話', '二上單元一', '小心眼'),
  ('神祕', '二上單元一', '小心眼'),
  ('難受', '二上單元一', '小心眼'),
  ('走廊', '二上單元一', '小心眼'),
  ('設計', '二上單元一', '小心眼'),
  ('生日卡', '二上單元一', '小心眼'),
  ('小心眼', '二上單元一', '小心眼'),
  ('錯怪', '二上單元一', '小心眼'),
  ('感動', '二上單元一', '小心眼'),
  ('接過', '二上單元一', '小心眼'),
  ('謝謝', '二上單元一', '小心眼'),
  ('禮物', '二上單元一', '小心眼'),
  ('負責', '二上單元一', '買食物'),
  ('主動', '二上單元一', '買食物'),
  ('要求', '二上單元一', '買食物'),
  ('反反覆覆', '二上單元一', '買食物'),
  ('汽水', '二上單元一', '買食物'),
  ('勇氣', '二上單元一', '買食物'),
  ('結結巴巴', '二上單元一', '買食物'),
  ('終於', '二上單元一', '買食物'),
  ('快步', '二上單元一', '買食物'),
  ('慌忙', '二上單元一', '買食物'),
  ('轉身', '二上單元一', '買食物'),
  ('悄悄', '二上單元一', '買食物'),
  ('剛才', '二上單元一', '買食物'),
  ('表現', '二上單元一', '買食物'),
  ('經歷', '二上單元一', '買食物'),
  ('緊張', '二上單元一', '買食物'),
  ('露珠', '二上單元二', '露珠'),
  ('早晨', '二上單元二', '露珠'),
  ('花園', '二上單元二', '露珠'),
  ('閃閃發光', '二上單元二', '露珠'),
  ('珍珠', '二上單元二', '露珠'),
  ('晶亮', '二上單元二', '露珠'),
  ('怎麼', '二上單元二', '露珠'),
  ('綠油油', '二上單元二', '露珠'),
  ('葉子', '二上單元二', '露珠'),
  ('紅豔豔', '二上單元二', '露珠'),
  ('花瓣', '二上單元二', '露珠'),
  ('帆船', '二上單元二', '風在哪兒'),
  ('行駛', '二上單元二', '風在哪兒'),
  ('舞蹈', '二上單元二', '風在哪兒'),
  ('搖晃', '二上單元二', '風在哪兒'),
  ('風鈴', '二上單元二', '風在哪兒'),
  ('好聽', '二上單元二', '風在哪兒'),
  ('翻開', '二上單元二', '風在哪兒'),
  ('揮動', '二上單元二', '風在哪兒'),
  ('扇子', '二上單元二', '風在哪兒'),
  ('大地', '二上單元二', '在海裏'),
  ('多姿多彩', '二上單元二', '在海裏'),
  ('花草樹木', '二上單元二', '在海裏'),
  ('海洋', '二上單元二', '在海裏'),
  ('植物', '二上單元二', '在海裏'),
  ('千姿百態', '二上單元二', '在海裏'),
  ('獅子', '二上單元二', '在海裏'),
  ('大象', '二上單元二', '在海裏'),
  ('飛快', '二上單元二', '在海裏'),
  ('不過', '二上單元二', '在海裏'),
  ('搖擺', '二上單元二', '在海裏'),
  ('草原', '二上單元二', '在海裏'),
  ('金黃', '二上單元二', '在海裏'),
  ('笑臉', '二上單元二', '在海裏'),
  ('賀卡', '二上單元二', '多彩的賀卡'),
  ('森林', '二上單元二', '多彩的賀卡'),
  ('祝願', '二上單元二', '多彩的賀卡'),
  ('樹苗', '二上單元二', '多彩的賀卡'),
  ('茁壯成長', '二上單元二', '多彩的賀卡'),
  ('歡樂', '二上單元二', '多彩的賀卡'),
  ('藍天', '二上單元二', '多彩的賀卡'),
  ('呼喚', '二上單元二', '多彩的賀卡'),
  ('飛翔', '二上單元二', '多彩的賀卡'),
  ('白鴿', '二上單元二', '多彩的賀卡'),
  ('貝殻', '二上單元二', '多彩的賀卡'),
  ('風帆', '二上單元二', '多彩的賀卡'),
  ('祕密', '二上單元二', '多彩的賀卡'),
  ('探索', '二上單元二', '多彩的賀卡'),
  ('珍惜', '二上單元二', '多彩的賀卡'),
  ('創造', '二上單元二', '多彩的賀卡'),
  ('未來', '二上單元二', '多彩的賀卡'),
  ('田野', '二上單元二', '找春天'),
  ('尋找', '二上單元二', '找春天'),
  ('害羞', '二上單元二', '找春天'),
  ('姑娘', '二上單元二', '找春天'),
  ('躲躲藏藏', '二上單元二', '找春天'),
  ('眉毛', '二上單元二', '找春天'),
  ('嫩芽', '二上單元二', '找春天'),
  ('音符', '二上單元二', '找春天'),
  ('解凍', '二上單元二', '找春天'),
  ('琴聲', '二上單元二', '找春天'),
  ('盪秋千', '二上單元二', '找春天'),
  ('風箏', '二上單元二', '找春天'),
  ('枝頭', '二上單元二', '找春天'),
  ('烏鴉', '二下單元一', '狐狸和烏鴉'),
  ('食物', '二下單元一', '狐狸和烏鴉'),
  ('不禁', '二下單元一', '狐狸和烏鴉'),
  ('孩子', '二下單元一', '狐狸和烏鴉'),
  ('辛辛苦苦', '二下單元一', '狐狸和烏鴉'),
  ('口水直流', '二下單元一', '狐狸和烏鴉'),
  ('主意', '二下單元一', '狐狸和烏鴉'),
  ('美妙', '二下單元一', '狐狸和烏鴉'),
  ('得意', '二下單元一', '狐狸和烏鴉'),
  ('急忙', '二下單元一', '狐狸和烏鴉'),
  ('已經', '二下單元一', '狐狸和烏鴉'),
  ('冬天', '二下單元一', '騾子和冰'),
  ('特別', '二下單元一', '騾子和冰'),
  ('寒冷', '二下單元一', '騾子和冰'),
  ('幸福', '二下單元一', '騾子和冰'),
  ('遠道而來', '二下單元一', '騾子和冰'),
  ('張望', '二下單元一', '騾子和冰'),
  ('摔倒', '二下單元一', '騾子和冰'),
  ('禮貌', '二下單元一', '騾子和冰'),
  ('粗魯', '二下單元一', '騾子和冰'),
  ('身體', '二下單元一', '騾子和冰'),
  ('温度', '二下單元一', '騾子和冰'),
  ('融化', '二下單元一', '騾子和冰'),
  ('冰冷', '二下單元一', '騾子和冰'),
  ('應該', '二下單元一', '騾子和冰'),
  ('聽從', '二下單元一', '騾子和冰'),
  ('勸告', '二下單元一', '騾子和冰'),
  ('破裂', '二下單元一', '騾子和冰'),
  ('寂寞', '二下單元一', '蜘蛛開店'),
  ('無聊', '二下單元一', '蜘蛛開店'),
  ('決定', '二下單元一', '蜘蛛開店'),
  ('商店', '二下單元一', '蜘蛛開店'),
  ('簡單', '二下單元一', '蜘蛛開店'),
  ('招牌', '二下單元一', '蜘蛛開店'),
  ('顧客', '二下單元一', '蜘蛛開店'),
  ('工夫', '二下單元一', '蜘蛛開店'),
  ('終於', '二下單元一', '蜘蛛開店'),
  ('圍巾', '二下單元一', '蜘蛛開店'),
  ('匆忙', '二下單元一', '蜘蛛開店'),
  ('原來', '二下單元一', '蜘蛛開店'),
  ('連蹦帶跳', '二下單元一', '小馬過河'),
  ('願意', '二下單元一', '小馬過河'),
  ('四周', '二下單元一', '小馬過河'),
  ('吃驚', '二下單元一', '小馬過河'),
  ('夥伴', '二下單元一', '小馬過河'),
  ('認真', '二下單元一', '小馬過河'),
  ('知道', '二下單元一', '小馬過河'),
  ('到底', '二下單元一', '小馬過河'),
  ('親切', '二下單元一', '小馬過河'),
  ('小心', '二下單元一', '小馬過河'),
  ('連忙', '二下單元一', '小馬過河'),
  ('動物', '二下單元一', '動物王國開大會'),
  ('王國', '二下單元一', '動物王國開大會'),
  ('老虎', '二下單元一', '動物王國開大會'),
  ('狗熊', '二下單元一', '動物王國開大會'),
  ('通知', '二下單元一', '動物王國開大會'),
  ('喇叭', '二下單元一', '動物王國開大會'),
  ('注意', '二下單元一', '動物王國開大會'),
  ('道理', '二下單元一', '動物王國開大會'),
  ('腦袋', '二下單元一', '動物王國開大會'),
  ('廣場', '二下單元一', '動物王國開大會'),
  ('準時', '二下單元一', '動物王國開大會'),
  ('明白', '二下單元一', '動物王國開大會'),
  ('因為', '二下單元一', '動物王國開大會'),
  ('地點', '二下單元一', '動物王國開大會'),
  ('夜景', '二下單元二', '東方之珠'),
  ('拍照', '二下單元二', '東方之珠'),
  ('錄像', '二下單元二', '東方之珠'),
  ('興奮', '二下單元二', '東方之珠'),
  ('高樓大廈', '二下單元二', '東方之珠'),
  ('兩岸', '二下單元二', '東方之珠'),
  ('路燈', '二下單元二', '東方之珠'),
  ('柔和', '二下單元二', '東方之珠'),
  ('五顏六色', '二下單元二', '東方之珠'),
  ('欣賞', '二下單元二', '東方之珠'),
  ('衣裳', '二下單元二', '東方之珠'),
  ('不斷', '二下單元二', '東方之珠'),
  ('目的地', '二下單元二', '遊海洋公園'),
  ('乘搭', '二下單元二', '遊海洋公園'),
  ('探望', '二下單元二', '遊海洋公園'),
  ('肚子', '二下單元二', '遊海洋公園'),
  ('動聽', '二下單元二', '遊海洋公園'),
  ('午飯', '二下單元二', '遊海洋公園'),
  ('表演', '二下單元二', '遊海洋公園'),
  ('配合', '二下單元二', '遊海洋公園'),
  ('列車', '二下單元二', '遊海洋公園'),
  ('各種', '二下單元二', '遊海洋公園'),
  ('訓練', '二下單元二', '遊海洋公園'),
  ('觀賞', '二下單元二', '遊海洋公園'),
  ('遊戲', '二下單元二', '遊海洋公園'),
  ('哈哈大笑', '二下單元二', '遊海洋公園'),
  ('依依不捨', '二下單元二', '遊海洋公園'),
  ('總是', '二下單元二', '香噴噴的夢'),
  ('下班', '二下單元二', '香噴噴的夢'),
  ('西裝', '二下單元二', '香噴噴的夢'),
  ('圍裙', '二下單元二', '香噴噴的夢'),
  ('廚房', '二下單元二', '香噴噴的夢'),
  ('專心', '二下單元二', '香噴噴的夢'),
  ('形狀', '二下單元二', '香噴噴的夢'),
  ('雞蛋', '二下單元二', '香噴噴的夢'),
  ('回來', '二下單元二', '香噴噴的夢'),
  ('晚餐', '二下單元二', '香噴噴的夢'),
  ('餃子', '二下單元二', '歡歡喜喜包餃子'),
  ('情景', '二下單元二', '歡歡喜喜包餃子'),
  ('吸引', '二下單元二', '歡歡喜喜包餃子'),
  ('趕緊', '二下單元二', '歡歡喜喜包餃子'),
  ('隊伍', '二下單元二', '歡歡喜喜包餃子'),
  ('發現', '二下單元二', '歡歡喜喜包餃子'),
  ('口袋', '二下單元二', '歡歡喜喜包餃子'),
  ('難看', '二下單元二', '歡歡喜喜包餃子'),
  ('學問', '二下單元二', '歡歡喜喜包餃子'),
  ('熱騰騰', '二下單元二', '歡歡喜喜包餃子'),
  ('爺爺', '二下單元二', '歡歡喜喜包餃子'),
  ('津津有味', '二下單元二', '歡歡喜喜包餃子'),
  ('滿足', '二下單元二', '歡歡喜喜包餃子'),
  ('笑容', '二下單元二', '歡歡喜喜包餃子'),
  ('幫忙', '二下單元二', '歡歡喜喜包餃子'),
  ('小心翼翼', '二下單元二', '歡歡喜喜包餃子'),
  ('隊伍', '二下單元二', '歡歡喜喜包餃子'),
  ('竟然', '二下單元二', '歡歡喜喜包餃子'),
  ('放假', '二下單元二', '美味的粽子'),
  ('祖母', '二下單元二', '美味的粽子'),
  ('粽子', '二下單元二', '美味的粽子'),
  ('首先', '二下單元二', '美味的粽子'),
  ('示範', '二下單元二', '美味的粽子'),
  ('然後', '二下單元二', '美味的粽子'),
  ('接着', '二下單元二', '美味的粽子'),
  ('最後', '二下單元二', '美味的粽子'),
  ('完成', '二下單元二', '美味的粽子'),
  ('講述', '二下單元二', '美味的粽子'),
  ('節日', '二下單元二', '美味的粽子'),
  ('食品', '二下單元二', '美味的粽子'),
  ('香噴噴', '二下單元二', '美味的粽子'),
  ('熱呼呼', '二下單元二', '美味的粽子'),
  ('美味', '二下單元二', '美味的粽子'),
  ('綠豆', '二下單元二', '美味的粽子'),
  ('放假', '三上單元一', '慧娟怎樣長大'),
  ('起牀', '三上單元一', '慧娟怎樣長大'),
  ('高跟鞋', '三上單元一', '慧娟怎樣長大'),
  ('眼鏡', '三上單元一', '慧娟怎樣長大'),
  ('項鏈', '三上單元一', '慧娟怎樣長大'),
  ('胡鬧', '三上單元一', '慧娟怎樣長大'),
  ('不算', '三上單元一', '慧娟怎樣長大'),
  ('辦法', '三上單元一', '慧娟怎樣長大'),
  ('掃帚', '三上單元一', '慧娟怎樣長大'),
  ('客廳', '三上單元一', '慧娟怎樣長大'),
  ('灰塵', '三上單元一', '慧娟怎樣長大'),
  ('驚喜', '三上單元一', '慧娟怎樣長大'),
  ('全部', '三上單元一', '慧娟怎樣長大'),
  ('診所', '三上單元一', '拔牙'),
  ('醫生', '三上單元一', '拔牙'),
  ('驚慌', '三上單元一', '拔牙'),
  ('好奇', '三上單元一', '拔牙'),
  ('温柔', '三上單元一', '拔牙'),
  ('安慰', '三上單元一', '拔牙'),
  ('仍然', '三上單元一', '拔牙'),
  ('驅除', '三上單元一', '拔牙'),
  ('恐慌', '三上單元一', '拔牙'),
  ('注射', '三上單元一', '拔牙'),
  ('耐心', '三上單元一', '拔牙'),
  ('稱讚', '三上單元一', '拔牙'),
  ('勇敢', '三上單元一', '拔牙'),
  ('並且', '三上單元一', '拔牙'),
  ('保護', '三上單元一', '拔牙'),
  ('輕鬆', '三上單元一', '拔牙'),
  ('鮮豔', '三上單元一', '一束鮮艷的花'),
  ('愛惜', '三上單元一', '一束鮮艷的花'),
  ('捨不得', '三上單元一', '一束鮮艷的花'),
  ('比賽', '三上單元一', '一束鮮艷的花'),
  ('建議', '三上單元一', '一束鮮艷的花'),
  ('表達', '三上單元一', '一束鮮艷的花'),
  ('盛開', '三上單元一', '一束鮮艷的花'),
  ('求助', '三上單元一', '一束鮮艷的花'),
  ('不好意思', '三上單元一', '一束鮮艷的花'),
  ('胡亂', '三上單元一', '一束鮮艷的花'),
  ('七嘴八舌', '三上單元一', '一束鮮艷的花'),
  ('肯定', '三上單元一', '一束鮮艷的花'),
  ('冠軍', '三上單元一', '一束鮮艷的花'),
  ('不安', '三上單元一', '一束鮮艷的花'),
  ('行為', '三上單元一', '一束鮮艷的花'),
  ('自私', '三上單元一', '一束鮮艷的花'),
  ('於是', '三上單元一', '一束鮮艷的花'),
  ('重新', '三上單元一', '一束鮮艷的花'),
  ('電視劇', '三上單元一', '上默書課'),
  ('精彩', '三上單元一', '上默書課'),
  ('温習', '三上單元一', '上默書課'),
  ('改期', '三上單元一', '上默書課'),
  ('唸唸有詞', '三上單元一', '上默書課'),
  ('記憶', '三上單元一', '上默書課'),
  ('內容', '三上單元一', '上默書課'),
  ('因為', '三上單元一', '上默書課'),
  ('請假', '三上單元一', '上默書課'),
  ('代課', '三上單元一', '上默書課'),
  ('宣佈', '三上單元一', '上默書課'),
  ('立即', '三上單元一', '上默書課'),
  ('瘦弱', '三上單元一', '上默書課'),
  ('一聲不響', '三上單元一', '上默書課'),
  ('通紅', '三上單元一', '上默書課'),
  ('相信', '三上單元一', '上默書課'),
  ('清澈', '三上單元一', '清澈的湖水'),
  ('兩側', '三上單元一', '清澈的湖水'),
  ('波紋', '三上單元一', '清澈的湖水'),
  ('麵包', '三上單元一', '清澈的湖水'),
  ('展翅欲飛', '三上單元一', '清澈的湖水'),
  ('雄鷹', '三上單元一', '清澈的湖水'),
  ('賽跑', '三上單元一', '清澈的湖水'),
  ('變幻', '三上單元一', '清澈的湖水'),
  ('消失', '三上單元一', '清澈的湖水'),
  ('皺紋', '三上單元一', '清澈的湖水'),
  ('不滿', '三上單元一', '清澈的湖水'),
  ('企盼', '三上單元一', '清澈的湖水'),
  ('目光', '三上單元一', '清澈的湖水'),
  ('跨步', '三上單元一', '清澈的湖水'),
  ('穩穩當當', '三上單元二', '曹沖稱象'),
  ('柱子', '三上單元二', '曹沖稱象'),
  ('議論', '三上單元二', '曹沖稱象'),
  ('重量', '三上單元二', '曹沖稱象'),
  ('一本正經', '三上單元二', '曹沖稱象'),
  ('微笑', '三上單元二', '曹沖稱象'),
  ('果然', '三上單元二', '曹沖稱象'),
  ('佩服', '三上單元二', '曹沖稱象'),
  ('讚歎', '三上單元二', '曹沖稱象'),
  ('年紀', '三上單元二', '曹沖稱象'),
  ('聰明', '三上單元二', '曹沖稱象'),
  ('了不起', '三上單元二', '曹沖稱象'),
  ('人山人海', '三上單元二', '王戎智捉人販子'),
  ('東張西望', '三上單元二', '王戎智捉人販子'),
  ('五彩繽紛', '三上單元二', '王戎智捉人販子'),
  ('眼花繚亂', '三上單元二', '王戎智捉人販子'),
  ('目不暇給', '三上單元二', '王戎智捉人販子'),
  ('僻靜', '三上單元二', '王戎智捉人販子'),
  ('反而', '三上單元二', '王戎智捉人販子'),
  ('竟然', '三上單元二', '王戎智捉人販子'),
  ('提防', '三上單元二', '王戎智捉人販子'),
  ('拐賣', '三上單元二', '王戎智捉人販子'),
  ('擁擠', '三上單元二', '王戎智捉人販子'),
  ('雖然', '三上單元二', '王戎智捉人販子'),
  ('帽子', '三上單元二', '王戎智捉人販子'),
  ('士兵', '三上單元二', '王戎智捉人販子'),
  ('拐騙', '三上單元二', '王戎智捉人販子'),
  ('奪門而逃', '三上單元二', '剃頭大師'),
  ('怒視', '三上單元二', '剃頭大師'),
  ('抗議', '三上單元二', '剃頭大師'),
  ('痛苦', '三上單元二', '剃頭大師'),
  ('習慣', '三上單元二', '剃頭大師'),
  ('吃盡苦頭', '三上單元二', '剃頭大師'),
  ('耿耿於懷', '三上單元二', '剃頭大師'),
  ('折磨', '三上單元二', '剃頭大師'),
  ('央求', '三上單元二', '剃頭大師'),
  ('答應', '三上單元二', '剃頭大師'),
  ('願意', '三上單元二', '剃頭大師'),
  ('隨便', '三上單元二', '剃頭大師'),
  ('處置', '三上單元二', '剃頭大師'),
  ('發誓', '三上單元二', '剃頭大師'),
  ('熟練', '三上單元二', '剃頭大師'),
  ('優秀', '三上單元二', '剃頭大師'),
  ('顧客', '三上單元二', '剃頭大師'),
  ('倒霉', '三上單元二', '剃頭大師'),
  ('頓時', '三上單元二', '遙控車壞了'),
  ('受傷', '三上單元二', '遙控車壞了'),
  ('憤怒', '三上單元二', '遙控車壞了'),
  ('哇哇大哭', '三上單元二', '遙控車壞了'),
  ('修理', '三上單元二', '遙控車壞了'),
  ('糖果', '三上單元二', '遙控車壞了'),
  ('煙消雲散', '三上單元二', '遙控車壞了'),
  ('心平氣和', '三上單元二', '遙控車壞了'),
  ('解決', '三上單元二', '遙控車壞了'),
  ('問題', '三上單元二', '遙控車壞了'),
  ('提早', '三上單元二', '我的球迷哥哥'),
  ('姿勢', '三上單元二', '我的球迷哥哥'),
  ('捶胸頓足', '三上單元二', '我的球迷哥哥'),
  ('嘮叨', '三上單元二', '我的球迷哥哥'),
  ('從此', '三上單元二', '我的球迷哥哥'),
  ('不許', '三上單元二', '我的球迷哥哥'),
  ('不但', '三上單元二', '我的球迷哥哥'),
  ('時機', '三上單元二', '我的球迷哥哥'),
  ('靈巧', '三上單元二', '我的球迷哥哥'),
  ('迅速', '三上單元二', '我的球迷哥哥'),
  ('左穿右插', '三上單元二', '我的球迷哥哥'),
  ('周圍', '三上單元二', '我的球迷哥哥'),
  ('出色', '三上單元二', '我的球迷哥哥'),
  ('爭光', '三上單元二', '我的球迷哥哥'),
  ('支持', '三上單元二', '我的球迷哥哥'),
  ('名字', '三下單元一', '我的名字叫做貓'),
  ('家族', '三下單元一', '我的名字叫做貓'),
  ('兇猛', '三下單元一', '我的名字叫做貓'),
  ('同類', '三下單元一', '我的名字叫做貓'),
  ('或者', '三下單元一', '我的名字叫做貓'),
  ('漆黑', '三下單元一', '我的名字叫做貓'),
  ('本領', '三下單元一', '我的名字叫做貓'),
  ('厲害', '三下單元一', '我的名字叫做貓'),
  ('因素', '三下單元一', '我的名字叫做貓'),
  ('瞳孔', '三下單元一', '我的名字叫做貓'),
  ('強弱', '三下單元一', '我的名字叫做貓'),
  ('縮小', '三下單元一', '我的名字叫做貓'),
  ('靈敏', '三下單元一', '我的名字叫做貓'),
  ('探路', '三下單元一', '我的名字叫做貓'),
  ('獵物', '三下單元一', '我的名字叫做貓'),
  ('因此', '三下單元一', '我的名字叫做貓'),
  ('捕捉', '三下單元一', '我的名字叫做貓'),
  ('戰無不勝', '三下單元一', '我的名字叫做貓'),
  ('難怪', '三下單元一', '我的名字叫做貓'),
  ('造型', '三下單元一', '綠樹枱燈'),
  ('茂盛', '三下單元一', '綠樹枱燈'),
  ('彷彿', '三下單元一', '綠樹枱燈'),
  ('氣息', '三下單元一', '綠樹枱燈'),
  ('圖案', '三下單元一', '綠樹枱燈'),
  ('主幹', '三下單元一', '綠樹枱燈'),
  ('別緻', '三下單元一', '綠樹枱燈'),
  ('設計', '三下單元一', '綠樹枱燈'),
  ('猶如', '三下單元一', '綠樹枱燈'),
  ('獨特', '三下單元一', '綠樹枱燈'),
  ('開關', '三下單元一', '綠樹枱燈'),
  ('提醒', '三下單元一', '綠樹枱燈'),
  ('代表', '三下單元一', '綠樹枱燈'),
  ('選擇', '三下單元一', '綠樹枱燈'),
  ('方便', '三下單元一', '綠樹枱燈'),
  ('實用', '三下單元一', '綠樹枱燈'),
  ('樹幹', '三下單元一', '綠樹枱燈'),
  ('故鄉', '三下單元一', '我愛故鄉的楊梅'),
  ('貪婪', '三下單元一', '我愛故鄉的楊梅'),
  ('甘露', '三下單元一', '我愛故鄉的楊梅'),
  ('狹長', '三下單元一', '我愛故鄉的楊梅'),
  ('楊梅', '三下單元一', '我愛故鄉的楊梅'),
  ('桂圓', '三下單元一', '我愛故鄉的楊梅'),
  ('舌尖', '三下單元一', '我愛故鄉的楊梅'),
  ('細膩', '三下單元一', '我愛故鄉的楊梅'),
  ('柔軟', '三下單元一', '我愛故鄉的楊梅'),
  ('雖然', '三下單元一', '我愛故鄉的楊梅'),
  ('幾乎', '三下單元一', '我愛故鄉的楊梅'),
  ('豆腐', '三下單元一', '我愛故鄉的楊梅'),
  ('熟透', '三下單元一', '我愛故鄉的楊梅'),
  ('演奏', '三下單元一', '大自然的聲音'),
  ('季節', '三下單元一', '大自然的聲音'),
  ('呢喃細語', '三下單元一', '大自然的聲音'),
  ('激動', '三下單元一', '大自然的聲音'),
  ('充滿', '三下單元一', '大自然的聲音'),
  ('威力', '三下單元一', '大自然的聲音'),
  ('熱鬧', '三下單元一', '大自然的聲音'),
  ('滙聚', '三下單元一', '大自然的聲音'),
  ('洶湧澎湃', '三下單元一', '大自然的聲音'),
  ('輕快', '三下單元一', '大自然的聲音'),
  ('波瀾壯闊', '三下單元一', '大自然的聲音'),
  ('打擊', '三下單元一', '大自然的聲音'),
  ('樂曲', '三下單元一', '大自然的聲音'),
  ('鑰匙', '三下單元一', '秋天的雨'),
  ('留意', '三下單元一', '秋天的雨'),
  ('顏料', '三下單元一', '秋天的雨'),
  ('炎熱', '三下單元一', '秋天的雨'),
  ('郵票', '三下單元一', '秋天的雨'),
  ('涼爽', '三下單元一', '秋天的雨'),
  ('你擠我碰', '三下單元一', '秋天的雨'),
  ('頻頻點頭', '三下單元一', '秋天的雨'),
  ('香甜', '三下單元一', '秋天的雨'),
  ('糧食', '三下單元一', '秋天的雨'),
  ('準備', '三下單元一', '秋天的雨'),
  ('豐收', '三下單元一', '秋天的雨'),
  ('歡樂', '三下單元一', '秋天的雨'),
  ('温柔', '三下單元一', '秋天的雨'),
  ('燦爛', '三下單元二', '參觀青馬大橋'),
  ('參觀', '三下單元二', '參觀青馬大橋'),
  ('大橋', '三下單元二', '參觀青馬大橋'),
  ('目的地', '三下單元二', '參觀青馬大橋'),
  ('模型', '三下單元二', '參觀青馬大橋'),
  ('過程', '三下單元二', '參觀青馬大橋'),
  ('觀察', '三下單元二', '參觀青馬大橋'),
  ('外形', '三下單元二', '參觀青馬大橋'),
  ('橫臥', '三下單元二', '參觀青馬大橋'),
  ('鐵路', '三下單元二', '參觀青馬大橋'),
  ('夕陽', '三下單元二', '參觀青馬大橋'),
  ('氣勢宏偉', '三下單元二', '參觀青馬大橋'),
  ('自豪', '三下單元二', '參觀青馬大橋'),
  ('名滿天下', '三下單元二', '參觀青馬大橋'),
  ('親手', '三下單元二', '參觀青馬大橋'),
  ('古色古香', '三下單元二', '遊迪士尼樂園'),
  ('商店', '三下單元二', '遊迪士尼樂園'),
  ('遙遙相對', '三下單元二', '遊迪士尼樂園'),
  ('聞名', '三下單元二', '遊迪士尼樂園'),
  ('遊樂設施', '三下單元二', '遊迪士尼樂園'),
  ('風土人情', '三下單元二', '遊迪士尼樂園'),
  ('悠揚', '三下單元二', '遊迪士尼樂園'),
  ('載歌載舞', '三下單元二', '遊迪士尼樂園'),
  ('效果', '三下單元二', '遊迪士尼樂園'),
  ('意想不到', '三下單元二', '遊迪士尼樂園'),
  ('依依不捨', '三下單元二', '遊迪士尼樂園'),
  ('世界', '三下單元二', '遊迪士尼樂園'),
  ('藝術', '三下單元二', '遊迪士尼樂園'),
  ('集中', '三下單元二', '花之路'),
  ('爭相開放', '三下單元二', '花之路'),
  ('層層疊疊', '三下單元二', '花之路'),
  ('香氣撲鼻', '三下單元二', '花之路'),
  ('購買', '三下單元二', '花之路'),
  ('小心翼翼', '三下單元二', '花之路'),
  ('芳香', '三下單元二', '花之路'),
  ('四面八方', '三下單元二', '花之路'),
  ('感覺', '三下單元二', '花之路'),
  ('延伸', '三下單元二', '花之路'),
  ('聞名中外', '三下單元二', '黃山奇石'),
  ('陡峭', '三下單元二', '黃山奇石'),
  ('秀麗', '三下單元二', '黃山奇石'),
  ('神奇', '三下單元二', '黃山奇石'),
  ('尤其', '三下單元二', '黃山奇石'),
  ('一動不動', '三下單元二', '黃山奇石'),
  ('翻滾', '三下單元二', '黃山奇石'),
  ('金光閃閃', '三下單元二', '黃山奇石'),
  ('著名', '三下單元二', '黃山奇石'),
  ('奇形怪狀', '三下單元二', '黃山奇石'),
  ('啼叫', '三下單元二', '黃山奇石'),
  ('風景優美', '三下單元二', '富饒的西沙羣島'),
  ('物產豐富', '三下單元二', '富饒的西沙羣島'),
  ('五光十色', '三下單元二', '富饒的西沙羣島'),
  ('瑰麗無比', '三下單元二', '富饒的西沙羣島'),
  ('高低不平', '三下單元二', '富饒的西沙羣島'),
  ('綻開', '三下單元二', '富饒的西沙羣島'),
  ('懶洋洋', '三下單元二', '富饒的西沙羣島'),
  ('威武', '三下單元二', '富饒的西沙羣島'),
  ('成群結隊', '三下單元二', '富饒的西沙羣島'),
  ('數不清', '三下單元二', '富饒的西沙羣島'),
  ('茂密', '三下單元二', '富饒的西沙羣島'),
  ('棲息', '三下單元二', '富饒的西沙羣島'),
  ('寶貴', '三下單元二', '富饒的西沙羣島'),
  ('祖祖輩輩', '三下單元二', '富饒的西沙羣島'),
  ('發展', '三下單元二', '富饒的西沙羣島'),
  ('肥料', '三下單元二', '富饒的西沙羣島'),
  ('堆積', '三下單元二', '富饒的西沙羣島'),
  ('建設', '三下單元二', '富饒的西沙羣島'),
  ('形影不離', '四上單元一', '小木船'),
  ('發生', '四上單元一', '小木船'),
  ('功課', '四上單元一', '小木船'),
  ('精緻', '四上單元一', '小木船'),
  ('故意', '四上單元一', '小木船'),
  ('絕不罷休', '四上單元一', '小木船'),
  ('體無完膚', '四上單元一', '小木船'),
  ('四分五裂', '四上單元一', '小木船'),
  ('氣惱', '四上單元一', '小木船'),
  ('委屈', '四上單元一', '小木船'),
  ('眼淚', '四上單元一', '小木船'),
  ('友誼', '四上單元一', '小木船'),
  ('破裂', '四上單元一', '小木船'),
  ('驚訝', '四上單元一', '小木船'),
  ('紀念', '四上單元一', '小木船'),
  ('歉意', '四上單元一', '小木船'),
  ('哽咽', '四上單元一', '小木船'),
  ('珍藏', '四上單元一', '小木船'),
  ('抽屜', '四上單元一', '小木船'),
  ('離開', '四上單元一', '掌聲'),
  ('殘疾', '四上單元一', '掌聲'),
  ('輪流', '四上單元一', '掌聲'),
  ('角落', '四上單元一', '掌聲'),
  ('猶豫', '四上單元一', '掌聲'),
  ('慢吞吞', '四上單元一', '掌聲'),
  ('注視', '四上單元一', '掌聲'),
  ('熱烈', '四上單元一', '掌聲'),
  ('持久', '四上單元一', '掌聲'),
  ('平息', '四上單元一', '掌聲'),
  ('情緒', '四上單元一', '掌聲'),
  ('講述', '四上單元一', '掌聲'),
  ('普通', '四上單元一', '掌聲'),
  ('永遠', '四上單元一', '掌聲'),
  ('忘記', '四上單元一', '掌聲'),
  ('歧視', '四上單元一', '掌聲'),
  ('鼓勵', '四上單元一', '掌聲'),
  ('禮物', '四上單元一', '保羅的自行車'),
  ('羨慕', '四上單元一', '保羅的自行車'),
  ('顯然', '四上單元一', '保羅的自行車'),
  ('寬裕', '四上單元一', '保羅的自行車'),
  ('驚歎', '四上單元一', '保羅的自行車'),
  ('希望', '四上單元一', '保羅的自行車'),
  ('不由自主', '四上單元一', '保羅的自行車'),
  ('敏捷', '四上單元一', '保羅的自行車'),
  ('期待', '四上單元一', '保羅的自行車'),
  ('分明', '四上單元一', '保羅的自行車'),
  ('麻煩', '四上單元一', '保羅的自行車'),
  ('將來', '四上單元一', '保羅的自行車'),
  ('濕潤', '四上單元一', '保羅的自行車'),
  ('喜悅', '四上單元一', '保羅的自行車'),
  ('給予', '四上單元一', '保羅的自行車'),
  ('夢想', '四上單元一', '愛的紙條'),
  ('退休', '四上單元一', '愛的紙條'),
  ('愛戴', '四上單元一', '愛的紙條'),
  ('傳統', '四上單元一', '愛的紙條'),
  ('抽獎', '四上單元一', '愛的紙條'),
  ('探親', '四上單元一', '愛的紙條'),
  ('邀請', '四上單元一', '愛的紙條'),
  ('出席', '四上單元一', '愛的紙條'),
  ('嘉賓', '四上單元一', '愛的紙條'),
  ('歡呼聲', '四上單元一', '愛的紙條'),
  ('震耳欲聾', '四上單元一', '愛的紙條'),
  ('擁抱', '四上單元一', '愛的紙條'),
  ('不約而同', '四上單元一', '愛的紙條'),
  ('放棄', '四上單元一', '愛的紙條'),
  ('機會', '四上單元一', '愛的紙條'),
  ('無私', '四上單元一', '愛的紙條'),
  ('善良', '四上單元一', '愛的紙條'),
  ('體現', '四上單元一', '愛的紙條'),
  ('旁邊', '四上單元二', '火燒雲'),
  ('乘涼', '四上單元二', '火燒雲'),
  ('變化', '四上單元二', '火燒雲'),
  ('跪着', '四上單元二', '火燒雲'),
  ('模糊', '四上單元二', '火燒雲'),
  ('忽然', '四上單元二', '火燒雲'),
  ('似乎', '四上單元二', '火燒雲'),
  ('鎮靜', '四上單元二', '火燒雲'),
  ('恍恍惚惚', '四上單元二', '火燒雲'),
  ('其實', '四上單元二', '火燒雲'),
  ('必須', '四上單元二', '火燒雲'),
  ('沉靜', '四上單元二', '火燒雲'),
  ('偏偏', '四上單元二', '火燒雲'),
  ('等待', '四上單元二', '火燒雲'),
  ('愛好', '四上單元二', '火燒雲'),
  ('嫩綠', '四上單元二', '美麗的小興安嶺'),
  ('融化', '四上單元二', '美麗的小興安嶺'),
  ('散步', '四上單元二', '美麗的小興安嶺'),
  ('擋住', '四上單元二', '美麗的小興安嶺'),
  ('視線', '四上單元二', '美麗的小興安嶺'),
  ('遮住', '四上單元二', '美麗的小興安嶺'),
  ('照射', '四上單元二', '美麗的小興安嶺'),
  ('宿舍', '四上單元二', '美麗的小興安嶺'),
  ('酸甜可口', '四上單元二', '美麗的小興安嶺'),
  ('收藏', '四上單元二', '美麗的小興安嶺'),
  ('來臨', '四上單元二', '美麗的小興安嶺'),
  ('誘人', '四上單元二', '美麗的小興安嶺'),
  ('寶庫', '四上單元二', '美麗的小興安嶺'),
  ('引人注目', '四上單元二', '美麗的香山'),
  ('遍佈', '四上單元二', '美麗的香山'),
  ('姿態萬千', '四上單元二', '美麗的香山'),
  ('絢麗異常', '四上單元二', '美麗的香山'),
  ('五彩斑斕', '四上單元二', '美麗的香山'),
  ('沉醉', '四上單元二', '美麗的香山'),
  ('空隙', '四上單元二', '美麗的香山'),
  ('流淌', '四上單元二', '美麗的香山'),
  ('耀眼', '四上單元二', '美麗的香山'),
  ('光芒', '四上單元二', '美麗的香山'),
  ('悠閒自在', '四上單元二', '美麗的香山'),
  ('爭奇鬥豔', '四上單元二', '美麗的香山'),
  ('竭力', '四上單元二', '美麗的香山'),
  ('散發', '四上單元二', '美麗的香山'),
  ('毫不示弱', '四上單元二', '美麗的香山'),
  ('回憶', '四上單元二', '美麗的香山'),
  ('構成', '四上單元二', '鄉下人家'),
  ('時令', '四上單元二', '鄉下人家'),
  ('順序', '四上單元二', '鄉下人家'),
  ('樸素', '四上單元二', '鄉下人家'),
  ('照例', '四上單元二', '鄉下人家'),
  ('率領', '四上單元二', '鄉下人家'),
  ('覓食', '四上單元二', '鄉下人家'),
  ('倘若', '四上單元二', '鄉下人家'),
  ('附近', '四上單元二', '鄉下人家'),
  ('情景', '四上單元二', '鄉下人家'),
  ('和諧', '四上單元二', '鄉下人家'),
  ('催眠曲', '四上單元二', '鄉下人家'),
  ('辛苦', '四上單元二', '鄉下人家'),
  ('夢鄉', '四上單元二', '鄉下人家'),
  ('不論', '四上單元二', '鄉下人家'),
  ('季節', '四上單元二', '鄉下人家'),
  ('迷人', '四上單元二', '鄉下人家'),
  ('歷史', '四下單元一', '奇妙的漢字'),
  ('創造', '四下單元一', '奇妙的漢字'),
  ('線條', '四下單元一', '奇妙的漢字'),
  ('粗略', '四下單元一', '奇妙的漢字'),
  ('形狀', '四下單元一', '奇妙的漢字'),
  ('表示', '四下單元一', '奇妙的漢字'),
  ('至於', '四下單元一', '奇妙的漢字'),
  ('意義', '四下單元一', '奇妙的漢字'),
  ('簡單', '四下單元一', '奇妙的漢字'),
  ('符號', '四下單元一', '奇妙的漢字'),
  ('組合', '四下單元一', '奇妙的漢字'),
  ('根本', '四下單元一', '奇妙的漢字'),
  ('顧名思義', '四下單元一', '奇妙的漢字'),
  ('領會', '四下單元一', '奇妙的漢字'),
  ('合併', '四下單元一', '奇妙的漢字'),
  ('產生', '四下單元一', '奇妙的漢字'),
  ('不僅', '四下單元一', '奇妙的漢字'),
  ('發明', '四下單元一', '紙的發明'),
  ('貢獻', '四下單元一', '紙的發明'),
  ('記錄', '四下單元一', '紙的發明'),
  ('笨重', '四下單元一', '紙的發明'),
  ('閱讀', '四下單元一', '紙的發明'),
  ('保存', '四下單元一', '紙的發明'),
  ('輕便', '四下單元一', '紙的發明'),
  ('普及', '四下單元一', '紙的發明'),
  ('製作', '四下單元一', '紙的發明'),
  ('粗糙', '四下單元一', '紙的發明'),
  ('書寫', '四下單元一', '紙的發明'),
  ('積累', '四下單元一', '紙的發明'),
  ('經驗', '四下單元一', '紙的發明'),
  ('價格', '四下單元一', '紙的發明'),
  ('滿足', '四下單元一', '紙的發明'),
  ('需要', '四下單元一', '紙的發明'),
  ('傳承', '四下單元一', '紙的發明'),
  ('促進', '四下單元一', '紙的發明'),
  ('影響', '四下單元一', '紙的發明'),
  ('便宜', '四下單元一', '紙的發明'),
  ('瞭解', '四下單元一', '夜間飛行的祕密'),
  ('捕捉', '四下單元一', '夜間飛行的祕密'),
  ('無論', '四下單元一', '夜間飛行的祕密'),
  ('靈巧', '四下單元一', '夜間飛行的祕密'),
  ('避開', '四下單元一', '夜間飛行的祕密'),
  ('難道', '四下單元一', '夜間飛行的祕密'),
  ('敏銳', '四下單元一', '夜間飛行的祕密'),
  ('實驗', '四下單元一', '夜間飛行的祕密'),
  ('橫七豎八', '四下單元一', '夜間飛行的祕密'),
  ('證明', '四下單元一', '夜間飛行的祕密'),
  ('配合', '四下單元一', '夜間飛行的祕密'),
  ('經過', '四下單元一', '夜間飛行的祕密'),
  ('反復', '四下單元一', '夜間飛行的祕密'),
  ('研究', '四下單元一', '夜間飛行的祕密'),
  ('揭開', '四下單元一', '夜間飛行的祕密'),
  ('傳播', '四下單元一', '夜間飛行的祕密'),
  ('障礙物', '四下單元一', '夜間飛行的祕密'),
  ('原理', '四下單元一', '夜間飛行的祕密'),
  ('類似', '四下單元一', '夜間飛行的祕密'),
  ('顯示', '四下單元一', '夜間飛行的祕密'),
  ('也許', '四下單元一', '什麼比獵豹的速度更快'),
  ('速度', '四下單元一', '什麼比獵豹的速度更快'),
  ('奔跑', '四下單元一', '什麼比獵豹的速度更快'),
  ('冠軍', '四下單元一', '什麼比獵豹的速度更快'),
  ('陸地', '四下單元一', '什麼比獵豹的速度更快'),
  ('俯衝', '四下單元一', '什麼比獵豹的速度更快'),
  ('移動', '四下單元一', '什麼比獵豹的速度更快'),
  ('擺脫', '四下單元一', '什麼比獵豹的速度更快'),
  ('浩瀚', '四下單元一', '什麼比獵豹的速度更快'),
  ('達到', '四下單元一', '什麼比獵豹的速度更快'),
  ('即使', '四下單元一', '什麼比獵豹的速度更快'),
  ('繼續', '四下單元一', '什麼比獵豹的速度更快'),
  ('呼嘯而過', '四下單元一', '什麼比獵豹的速度更快'),
  ('靜止', '四下單元一', '什麼比獵豹的速度更快'),
  ('物體', '四下單元一', '什麼比獵豹的速度更快'),
  ('難以置信', '四下單元一', '什麼比獵豹的速度更快'),
  ('任何', '四下單元一', '什麼比獵豹的速度更快'),
  ('率領', '四下單元二', '孫悟空，變變變！'),
  ('緝拿歸案', '四下單元二', '孫悟空，變變變！'),
  ('英勇善戰', '四下單元二', '孫悟空，變變變！'),
  ('應付', '四下單元二', '孫悟空，變變變！'),
  ('綽綽有餘', '四下單元二', '孫悟空，變變變！'),
  ('精疲力盡', '四下單元二', '孫悟空，變變變！'),
  ('把戲', '四下單元二', '孫悟空，變變變！'),
  ('避開', '四下單元二', '孫悟空，變變變！'),
  ('罷休', '四下單元二', '孫悟空，變變變！'),
  ('光禿禿', '四下單元二', '孫悟空，變變變！'),
  ('孤零零', '四下單元二', '孫悟空，變變變！'),
  ('可疑', '四下單元二', '孫悟空，變變變！'),
  ('大吃一驚', '四下單元二', '孫悟空，變變變！'),
  ('無影無蹤', '四下單元二', '孫悟空，變變變！'),
  ('攻打', '四下單元二', '諸葛亮巧佈空城計'),
  ('出師不利', '四下單元二', '諸葛亮巧佈空城計'),
  ('乘勝追擊', '四下單元二', '諸葛亮巧佈空城計'),
  ('吩咐', '四下單元二', '諸葛亮巧佈空城計'),
  ('掩護', '四下單元二', '諸葛亮巧佈空城計'),
  ('抵禦', '四下單元二', '諸葛亮巧佈空城計'),
  ('驚惶失措', '四下單元二', '諸葛亮巧佈空城計'),
  ('氣定神閒', '四下單元二', '諸葛亮巧佈空城計'),
  ('發號施令', '四下單元二', '諸葛亮巧佈空城計'),
  ('妥當', '四下單元二', '諸葛亮巧佈空城計'),
  ('悠揚', '四下單元二', '諸葛亮巧佈空城計'),
  ('埋伏', '四下單元二', '諸葛亮巧佈空城計'),
  ('謹慎', '四下單元二', '諸葛亮巧佈空城計'),
  ('判斷', '四下單元二', '諸葛亮巧佈空城計'),
  ('懊悔', '四下單元二', '諸葛亮巧佈空城計'),
  ('仰天長歎', '四下單元二', '諸葛亮巧佈空城計'),
  ('及時', '四下單元二', '扁鵲治病（白話文）'),
  ('治療', '四下單元二', '扁鵲治病（白話文）'),
  ('以免', '四下單元二', '扁鵲治病（白話文）'),
  ('不以為然', '四下單元二', '扁鵲治病（白話文）'),
  ('不痛不癢', '四下單元二', '扁鵲治病（白話文）'),
  ('惡化', '四下單元二', '扁鵲治病（白話文）'),
  ('無奈', '四下單元二', '扁鵲治病（白話文）'),
  ('原因', '四下單元二', '扁鵲治病（白話文）'),
  ('渾身', '四下單元二', '扁鵲治病（白話文）'),
  ('痛苦', '四下單元二', '扁鵲治病（白話文）'),
  ('疾病', '四下單元二', '扁鵲治病（白話文）'),
  ('缺點', '四下單元二', '扁鵲治病（白話文）'),
  ('採取', '四下單元二', '扁鵲治病（白話文）'),
  ('措施', '四下單元二', '扁鵲治病（白話文）'),
  ('否則', '四下單元二', '扁鵲治病（白話文）'),
  ('情況', '四下單元二', '扁鵲治病（白話文）'),
  ('病入膏肓', '四下單元二', '扁鵲治病（白話文）'),
  ('無藥可救', '四下單元二', '扁鵲治病（白話文）'),
  ('技術', '四下單元二', '紀昌學射'),
  ('巧妙', '四下單元二', '紀昌學射'),
  ('超過', '四下單元二', '紀昌學射'),
  ('織布', '四下單元二', '紀昌學射'),
  ('緊盯', '四下單元二', '紀昌學射'),
  ('報告', '四下單元二', '紀昌學射'),
  ('稱讚', '四下單元二', '紀昌學射'),
  ('明顯', '四下單元二', '紀昌學射'),
  ('目不轉睛', '四下單元二', '紀昌學射'),
  ('裝飾', '四下單元二', '紀昌學射'),
  ('舉辦', '五上單元一', '溜冰場上'),
  ('笨拙', '五上單元一', '溜冰場上'),
  ('四腳朝天', '五上單元一', '溜冰場上'),
  ('戰戰兢兢', '五上單元一', '溜冰場上'),
  ('跌倒', '五上單元一', '溜冰場上'),
  ('膽怯', '五上單元一', '溜冰場上'),
  ('頻繁', '五上單元一', '溜冰場上'),
  ('失誤', '五上單元一', '溜冰場上'),
  ('氣喘吁吁', '五上單元一', '溜冰場上'),
  ('東歪西倒', '五上單元一', '溜冰場上'),
  ('不禁', '五上單元一', '溜冰場上'),
  ('張皇失措', '五上單元一', '溜冰場上'),
  ('無動於衷', '五上單元一', '溜冰場上'),
  ('無可奈何', '五上單元一', '溜冰場上'),
  ('克服', '五上單元一', '溜冰場上'),
  ('恍然大悟', '五上單元一', '溜冰場上'),
  ('否則', '五上單元一', '溜冰場上'),
  ('啟示', '五上單元一', '溜冰場上'),
  ('獲益良多', '五上單元一', '溜冰場上'),
  ('毛病', '五上單元一', '爸爸的花兒落了（節選）'),
  ('懶惰', '五上單元一', '爸爸的花兒落了（節選）'),
  ('害羞', '五上單元一', '爸爸的花兒落了（節選）'),
  ('恐懼', '五上單元一', '爸爸的花兒落了（節選）'),
  ('勇氣', '五上單元一', '爸爸的花兒落了（節選）'),
  ('催促', '五上單元一', '爸爸的花兒落了（節選）'),
  ('哀求', '五上單元一', '爸爸的花兒落了（節選）'),
  ('命令', '五上單元一', '爸爸的花兒落了（節選）'),
  ('躲避', '五上單元一', '爸爸的花兒落了（節選）'),
  ('傷痕', '五上單元一', '爸爸的花兒落了（節選）'),
  ('遮蓋', '五上單元一', '爸爸的花兒落了（節選）'),
  ('原諒', '五上單元一', '爸爸的花兒落了（節選）'),
  ('緣故', '五上單元一', '爸爸的花兒落了（節選）'),
  ('禮貌', '五上單元一', '爸爸的花兒落了（節選）'),
  ('示意', '五上單元一', '爸爸的花兒落了（節選）'),
  ('徵求', '五上單元一', '爸爸的花兒落了（節選）'),
  ('同意', '五上單元一', '爸爸的花兒落了（節選）'),
  ('微笑', '五上單元一', '爸爸的花兒落了（節選）'),
  ('答應', '五上單元一', '爸爸的花兒落了（節選）'),
  ('維持', '五上單元一', '中彩那天'),
  ('拮据', '五上單元一', '中彩那天'),
  ('誠實', '五上單元一', '中彩那天'),
  ('精湛', '五上單元一', '中彩那天'),
  ('器重', '五上單元一', '中彩那天'),
  ('夢寐以求', '五上單元一', '中彩那天'),
  ('嶄新', '五上單元一', '中彩那天'),
  ('饋贈', '五上單元一', '中彩那天'),
  ('嚴肅', '五上單元一', '中彩那天'),
  ('悶悶不樂', '五上單元一', '中彩那天'),
  ('道德', '五上單元一', '中彩那天'),
  ('迷惑不解', '五上單元一', '中彩那天'),
  ('號碼', '五上單元一', '中彩那天'),
  ('辨別', '五上單元一', '中彩那天'),
  ('痕跡', '五上單元一', '中彩那天'),
  ('教誨', '五上單元一', '中彩那天'),
  ('附近', '五上單元一', '釣魚的啟示'),
  ('劇烈', '五上單元一', '釣魚的啟示'),
  ('抖動', '五上單元一', '釣魚的啟示'),
  ('操縱', '五上單元一', '釣魚的啟示'),
  ('掙扎', '五上單元一', '釣魚的啟示'),
  ('筋疲力盡', '五上單元一', '釣魚的啟示'),
  ('距離', '五上單元一', '釣魚的啟示'),
  ('急切', '五上單元一', '釣魚的啟示'),
  ('爭辯', '五上單元一', '釣魚的啟示'),
  ('乞求', '五上單元一', '釣魚的啟示'),
  ('沮喪', '五上單元一', '釣魚的啟示'),
  ('誘惑', '五上單元一', '釣魚的啟示'),
  ('抉擇', '五上單元一', '釣魚的啟示'),
  ('告誡', '五上單元一', '釣魚的啟示'),
  ('實踐', '五上單元一', '釣魚的啟示'),
  ('嚴格', '五上單元一', '釣魚的啟示'),
  ('終生', '五上單元一', '釣魚的啟示'),
  ('蹤影', '五上單元二', '霧鎖香江'),
  ('白茫茫', '五上單元二', '霧鎖香江'),
  ('雲霧', '五上單元二', '霧鎖香江'),
  ('專注', '五上單元二', '霧鎖香江'),
  ('也許', '五上單元二', '霧鎖香江'),
  ('籠罩', '五上單元二', '霧鎖香江'),
  ('隱隱約約', '五上單元二', '霧鎖香江'),
  ('輪廓', '五上單元二', '霧鎖香江'),
  ('遮蓋', '五上單元二', '霧鎖香江'),
  ('臉龐', '五上單元二', '霧鎖香江'),
  ('熟悉', '五上單元二', '霧鎖香江'),
  ('若隱若現', '五上單元二', '霧鎖香江'),
  ('儀態萬千', '五上單元二', '霧鎖香江'),
  ('怦然心動', '五上單元二', '霧鎖香江'),
  ('由衷', '五上單元二', '霧鎖香江'),
  ('和煦', '五上單元二', '霧鎖香江'),
  ('沐浴', '五上單元二', '霧鎖香江'),
  ('心曠神怡', '五上單元二', '霧鎖香江'),
  ('聞名於世', '五上單元二', '觀潮'),
  ('天下奇觀', '五上單元二', '觀潮'),
  ('據說', '五上單元二', '觀潮'),
  ('寬闊', '五上單元二', '觀潮'),
  ('屹立', '五上單元二', '觀潮'),
  ('昂首', '五上單元二', '觀潮'),
  ('人聲鼎沸', '五上單元二', '觀潮'),
  ('風平浪靜', '五上單元二', '觀潮'),
  ('逐漸', '五上單元二', '觀潮'),
  ('橫貫', '五上單元二', '觀潮'),
  ('浩浩蕩蕩', '五上單元二', '觀潮'),
  ('山崩地裂', '五上單元二', '觀潮'),
  ('顫動', '五上單元二', '觀潮'),
  ('依舊', '五上單元二', '觀潮'),
  ('風號浪吼', '五上單元二', '觀潮'),
  ('歎為觀止', '五上單元二', '觀潮'),
  ('遊覽', '五上單元二', '西湖風光'),
  ('記載', '五上單元二', '西湖風光'),
  ('修築', '五上單元二', '西湖風光'),
  ('映襯', '五上單元二', '西湖風光'),
  ('疏疏落落', '五上單元二', '西湖風光'),
  ('眉飛色舞', '五上單元二', '西湖風光'),
  ('亭亭玉立', '五上單元二', '西湖風光'),
  ('巍然聳立', '五上單元二', '西湖風光'),
  ('徒有虛名', '五上單元二', '西湖風光'),
  ('政府', '五上單元二', '西湖風光'),
  ('彌補', '五上單元二', '西湖風光'),
  ('行程', '五上單元二', '西湖風光'),
  ('遺憾', '五上單元二', '西湖風光'),
  ('環繞', '五上單元二', '西湖風光'),
  ('清幽淡雅', '五上單元二', '西湖風光'),
  ('百看不厭', '五上單元二', '西湖風光'),
  ('景致', '五上單元二', '西湖風光'),
  ('美不勝收', '五上單元二', '西湖風光'),
  ('流連忘返', '五上單元二', '西湖風光'),
  ('背誦', '五上單元二', '西湖風光'),
  ('波瀾壯闊', '五上單元二', '桂林山水'),
  ('無瑕', '五上單元二', '桂林山水'),
  ('攀登', '五上單元二', '桂林山水'),
  ('峯巒雄偉', '五上單元二', '桂林山水'),
  ('拔地而起', '五上單元二', '桂林山水'),
  ('奇峯羅列', '五上單元二', '桂林山水'),
  ('屏障', '五上單元二', '桂林山水'),
  ('栽倒', '五上單元二', '桂林山水'),
  ('圍繞', '五上單元二', '桂林山水'),
  ('倒映', '五上單元二', '桂林山水'),
  ('連綿不斷', '五上單元二', '桂林山水'),
  ('擴散', '五上單元二', '桂林山水'),
  ('畫卷', '五上單元二', '桂林山水'),
  ('骨碌碌', '五下單元一', '會捕食的植物'),
  ('搜索', '五下單元一', '會捕食的植物'),
  ('介紹', '五下單元一', '會捕食的植物'),
  ('濃密', '五下單元一', '會捕食的植物'),
  ('狹長', '五下單元一', '會捕食的植物'),
  ('色澤', '五下單元一', '會捕食的植物'),
  ('獵物', '五下單元一', '會捕食的植物'),
  ('誘捕', '五下單元一', '會捕食的植物'),
  ('賞心悅目', '五下單元一', '會捕食的植物'),
  ('守株待兔', '五下單元一', '會捕食的植物'),
  ('通常', '五下單元一', '會捕食的植物'),
  ('強烈', '五下單元一', '會捕食的植物'),
  ('濃郁', '五下單元一', '會捕食的植物'),
  ('自投羅網', '五下單元一', '會捕食的植物'),
  ('養分', '五下單元一', '會捕食的植物'),
  ('寬敞', '五下單元一', '鯨'),
  ('哺乳', '五下單元一', '鯨'),
  ('祖先', '五下單元一', '鯨'),
  ('屬於', '五下單元一', '鯨'),
  ('環境', '五下單元一', '鯨'),
  ('退化', '五下單元一', '鯨'),
  ('適應', '五下單元一', '鯨'),
  ('鋒利', '五下單元一', '鯨'),
  ('潛入', '五下單元一', '鯨'),
  ('傾斜', '五下單元一', '鯨'),
  ('特徵', '五下單元一', '鯨'),
  ('噴泉', '五下單元一', '鯨'),
  ('特徵', '五下單元一', '鯨'),
  ('甚至', '五下單元一', '鯨'),
  ('垂直', '五下單元一', '鯨'),
  ('兇猛', '五下單元一', '鯨'),
  ('壽命', '五下單元一', '鯨'),
  ('傳說', '五下單元一', '太陽'),
  ('寸草不生', '五下單元一', '太陽'),
  ('實際', '五下單元一', '太陽'),
  ('體積', '五下單元一', '太陽'),
  ('關係', '五下單元一', '太陽'),
  ('密切', '五下單元一', '太陽'),
  ('糧食', '五下單元一', '太陽'),
  ('繁殖', '五下單元一', '太陽'),
  ('生存', '五下單元一', '太陽'),
  ('下降', '五下單元一', '太陽'),
  ('流動', '五下單元一', '太陽'),
  ('殺菌', '五下單元一', '太陽'),
  ('預防', '五下單元一', '太陽'),
  ('治療', '五下單元一', '太陽'),
  ('疾病', '五下單元一', '太陽'),
  ('鋼鐵', '五下單元一', '太陽'),
  ('形成', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('推測', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('存在', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('分析', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('曾經', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('荒涼', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('突如其來', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('襲擊', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('遊盪', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('碰撞', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('家常便飯', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('豐富', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('持續', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('來源', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('誕生', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('潛藏', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('爆發', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('釋放', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('衝刷', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('痕跡', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('咆哮', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('孕育', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('缺陷', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('導致', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('足夠', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('渺茫', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('模樣', '五下單元一', '火星——地球的「孿生兄弟」'),
  ('進攻', '五下單元二', '將相和'),
  ('無價之寶', '五下單元二', '將相和'),
  ('召集', '五下單元二', '將相和'),
  ('商議', '五下單元二', '將相和'),
  ('機智', '五下單元二', '將相和'),
  ('理虧', '五下單元二', '將相和'),
  ('完好無缺', '五下單元二', '將相和'),
  ('絕口不提', '五下單元二', '將相和'),
  ('怒髮衝冠', '五下單元二', '將相和'),
  ('承諾', '五下單元二', '將相和'),
  ('得罪', '五下單元二', '將相和'),
  ('推辭', '五下單元二', '將相和'),
  ('擅長', '五下單元二', '將相和'),
  ('同歸於盡', '五下單元二', '將相和'),
  ('怒目圓睜', '五下單元二', '將相和'),
  ('毫不示弱', '五下單元二', '將相和'),
  ('削弱', '五下單元二', '將相和'),
  ('利益', '五下單元二', '將相和'),
  ('同心協力', '五下單元二', '將相和'),
  ('保衞', '五下單元二', '將相和'),
  ('威迫', '五下單元二', '廉頗和藺相如'),
  ('羞辱', '五下單元二', '廉頗和藺相如'),
  ('屢立戰功', '五下單元二', '廉頗和藺相如'),
  ('傲慢', '五下單元二', '廉頗和藺相如'),
  ('忍無可忍', '五下單元二', '廉頗和藺相如'),
  ('得罪', '五下單元二', '廉頗和藺相如'),
  ('不屑', '五下單元二', '廉頗和藺相如'),
  ('阻止', '五下單元二', '廉頗和藺相如'),
  ('謙讓', '五下單元二', '廉頗和藺相如'),
  ('揚長而去', '五下單元二', '廉頗和藺相如'),
  ('侵犯', '五下單元二', '廉頗和藺相如'),
  ('戰績顯赫', '五下單元二', '廉頗和藺相如'),
  ('忠心耿耿', '五下單元二', '廉頗和藺相如'),
  ('崇拜', '五下單元二', '廉頗和藺相如'),
  ('計較', '五下單元二', '廉頗和藺相如'),
  ('囉嗦', '五下單元二', '廉頗和藺相如'),
  ('慚愧', '五下單元二', '廉頗和藺相如'),
  ('吩咐', '五下單元二', '廉頗和藺相如'),
  ('繁榮昌盛', '五下單元二', '廉頗和藺相如'),
  ('才能', '五下單元二', '田忌賽馬'),
  ('將軍', '五下單元二', '田忌賽馬'),
  ('勝利', '五下單元二', '田忌賽馬'),
  ('當然', '五下單元二', '田忌賽馬'),
  ('為難', '五下單元二', '田忌賽馬'),
  ('胸有成竹', '五下單元二', '田忌賽馬'),
  ('疑惑', '五下單元二', '田忌賽馬'),
  ('規則', '五下單元二', '田忌賽馬'),
  ('淡定', '五下單元二', '田忌賽馬'),
  ('遙遙領先', '五下單元二', '田忌賽馬'),
  ('信服', '五下單元二', '田忌賽馬'),
  ('調整', '五下單元二', '田忌賽馬'),
  ('順序', '五下單元二', '田忌賽馬'),
  ('反敗為勝', '五下單元二', '田忌賽馬'),
  ('訪問', '五下單元二', '晏子使楚'),
  ('國勢強盛', '五下單元二', '晏子使楚'),
  ('侮辱', '五下單元二', '晏子使楚'),
  ('威風', '五下單元二', '晏子使楚'),
  ('到底', '五下單元二', '晏子使楚'),
  ('迎接', '五下單元二', '晏子使楚'),
  ('打發', '五下單元二', '晏子使楚'),
  ('規矩', '五下單元二', '晏子使楚'),
  ('招待', '五下單元二', '晏子使楚'),
  ('盜竊', '五下單元二', '晏子使楚'),
  ('沒出息', '五下單元二', '晏子使楚'),
  ('得意揚揚', '五下單元二', '晏子使楚'),
  ('面不改色', '五下單元二', '晏子使楚'),
  ('安居樂業', '五下單元二', '晏子使楚'),
  ('勞動', '五下單元二', '晏子使楚'),
  ('取笑', '五下單元二', '晏子使楚'),
  ('尊重', '五下單元二', '晏子使楚')
) AS t(word, level_2_tag, level_3_tag)
ON CONFLICT (wordlist_id, word, level_2_tag, level_3_tag) DO NOTHING;

COMMIT;

-- ========================================
-- 驗證導入結果
-- ========================================

DO $$
DECLARE
  v_wordlist_id UUID;
  v_word_count INT;
  v_unit_count INT;
  v_lesson_count INT;
  r RECORD;  -- 添加循環變量聲明
BEGIN
  -- 獲取詞表 ID
  SELECT id INTO v_wordlist_id
  FROM wordlists
  WHERE code = 'primary_chinese_2025';
  
  -- 統計詞彙數量
  SELECT COUNT(*) INTO v_word_count
  FROM wordlist_vocabulary
  WHERE wordlist_id = v_wordlist_id;
  
  -- 統計單元數量
  SELECT COUNT(DISTINCT level_2_tag) INTO v_unit_count
  FROM wordlist_vocabulary
  WHERE wordlist_id = v_wordlist_id;
  
  -- 統計課文數量
  SELECT COUNT(DISTINCT level_3_tag) INTO v_lesson_count
  FROM wordlist_vocabulary
  WHERE wordlist_id = v_wordlist_id;
  
  RAISE NOTICE '';
  RAISE NOTICE '========================================';
  RAISE NOTICE '✅ 導入完成！';
  RAISE NOTICE '========================================';
  RAISE NOTICE '';
  RAISE NOTICE '詞表信息：';
  RAISE NOTICE '  - 詞表 ID: %', v_wordlist_id;
  RAISE NOTICE '  - 總詞語數: %', v_word_count;
  RAISE NOTICE '  - 單元數量: %', v_unit_count;
  RAISE NOTICE '  - 課文數量: %', v_lesson_count;
  RAISE NOTICE '';
  
  -- 顯示各單元的詞彙數量
  RAISE NOTICE '各單元詞彙數量：';
  FOR r IN (
    SELECT 
      level_2_tag,
      COUNT(*) as count
    FROM wordlist_vocabulary
    WHERE wordlist_id = v_wordlist_id
    GROUP BY level_2_tag
    ORDER BY level_2_tag
  ) LOOP
    RAISE NOTICE '  - %: % 個詞語', r.level_2_tag, r.count;
  END LOOP;
END $$;

-- =====================================================
-- 使用說明
-- =====================================================
--
-- 在 Supabase Dashboard 的 SQL Editor 中：
-- 1. 複製此完整 SQL 腳本
-- 2. 貼上到 SQL Editor
-- 3. 點擊 "Run" 執行
-- 4. 查看執行結果和統計信息
--
-- 注意：
-- - 如果詞表已存在，會更新詞表信息
-- - 重複的詞彙會自動跳過（ON CONFLICT DO NOTHING）
-- - 執行時間取決於數據庫性能，約需 5-10 秒
--
-- =====================================================
