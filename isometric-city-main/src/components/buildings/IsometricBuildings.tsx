...
export const IsometricTile: React.FC<{ 
  ...
  const zoneColors: Record<ZoneType, string> = {
    living: 'rgba(76, 175, 80, 0.4)',      // 翰墨境（原 residential）
    market: 'rgba(33, 150, 243, 0.4)',     // 錦繡境（原 commercial）
    farming: 'rgba(255, 193, 7, 0.4)',     // 豆田境（原 industrial）
    none: 'transparent'
  };
    ...
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ display: 'block' }}>
      ...
      {zone && zone !== 'none' && (
        <polygon
          points={getTilePoints(w)}
          fill="none"
          stroke={zone === 'living' ? '#4CAF50' : zone === 'market' ? '#2196F3' : '#FFC107'}
          strokeWidth={1.5}
          strokeDasharray="4,2"
        />
      )}
