...
import { GameModeSwitch, ExploreOverlay } from '@/components/game/GameModeSwitch';

// Cargo type names for notifications
const CARGO_TYPE_NAMES = ['containers', 'bulk materials', 'oil'];

export default function Game({ onExit }: { onExit?: () => void }) {
  const { state, setTool, setActivePanel, addMoney, addNotification, setSpeed, gameMode } = useGame();
  ...
  // Desktop layout - 簡化版（無側邊欄，底部工具欄整合所有功能）
  return (
    <TooltipProvider>
      <div className="w-full h-full min-h-[720px] overflow-hidden bg-background flex flex-col">
        ...
        <div className="flex-1 relative overflow-visible">
          ...
          />
          {/* 底部工具欄（只在建造模式顯示） */}
          {gameMode === 'build' && <BottomToolbar onExit={onExit} />}
        </div>
