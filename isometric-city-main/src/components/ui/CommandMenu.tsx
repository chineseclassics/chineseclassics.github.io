...
// 輔助函數：安全獲取工具信息
function safeGetToolInfo(tool: Tool): { name: string; description: string; cost: number } | null {
...
function buildMenuItems(): MenuItem[] {
  ...
  // 系統工具（只包含 TOOL_INFO 中存在的工具）
  const toolsCategory: Tool[] = ['select', 'bulldoze', 'road', 'rail', 'subway', 'tree', 'water', 'grass'];
  toolsCategory.forEach(tool => {
    const info = safeGetToolInfo(tool);
    if (!info) return;
    items.push({
      id: tool,
      type: 'tool',
  ...
  // 區域規劃
  const zonesCategory: Tool[] = ['zone_living', 'zone_market', 'zone_farming', 'zone_dezone'];
  zonesCategory.forEach(tool => {
    ...
    items.push({
      ...
      cost: info.cost,
      category: 'zones',
      keywords: [info.name.toLowerCase(), tool, 'zone', 'zoning', '區域', '規劃'],
    });
  });
