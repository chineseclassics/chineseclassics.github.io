...
export function MobileTopBar({ 
  ...
  services: { health: number[][]; education: number[][]; power: boolean[][]; water: boolean[][] };
  onCloseTile: () => void;
  onExit?: () => void;
}) {
      ...
      <Card className="fixed top-0 left-0 right-0 z-40 rounded-none border-x-0 border-t-0 bg-card/95 backdrop-blur-sm safe-area-top">
        ...
        <div className="flex items-center justify-between px-3 py-1 border-t border-sidebar-border/50 bg-secondary/30">
          <div className="flex items-center gap-3">
            <DemandBar label="居" demand={stats.demand.living} color="text-green-500" />
            <DemandBar label="市" demand={stats.demand.market} color="text-blue-500" />
            <DemandBar label="田" demand={stats.demand.farming} color="text-amber-500" />
          </div>
          ...
          <div className="border-t border-sidebar-border/50 bg-gradient-to-b from-secondary/60 to-secondary/20 px-3 py-0.5 flex items-center gap-2 text-[10px]">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <div className={`w-2 h-2 rounded-full ${
                selectedTile.zone === 'living' ? 'bg-green-500' :
                selectedTile.zone === 'market' ? 'bg-blue-500' :
                selectedTile.zone === 'farming' ? 'bg-amber-500' : 'bg-muted-foreground/40'
              }`} />
          ...
          >
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">居民</span>
                <span className="text-sm font-mono text-foreground">{stats.population.toLocaleString()}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">月入</span>
