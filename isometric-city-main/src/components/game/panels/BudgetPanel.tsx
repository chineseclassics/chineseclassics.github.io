  ...
  return (
    <Dialog open={true} onOpenChange={() => setActivePanel('none')}>
      <DialogContent className="max-w-[500px]">
        <DialogHeader>
          <DialogTitle>財政預算</DialogTitle>
        </DialogHeader>
        
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4 pb-4 border-b border-border">
            <div>
              <div className="text-muted-foreground text-xs mb-1">月入</div>
              <div className="text-green-400 font-mono">{stats.income.toLocaleString()} 幣</div>
            </div>
            <div>
              <div className="text-muted-foreground text-xs mb-1">月支</div>
              <div className="text-red-400 font-mono">{stats.expenses.toLocaleString()} 幣</div>
            </div>
            <div>
              <div className="text-muted-foreground text-xs mb-1">淨額</div>
              <div className={`font-mono ${stats.income - stats.expenses >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                {(stats.income - stats.expenses).toLocaleString()} 幣
              </div>
            </div>
          </div>
