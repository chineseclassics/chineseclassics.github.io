...
export function drawAirplanes(
  ...
  // 飛鳥不使用精靈圖，直接使用 fallback 繪製
  const planeSprite = null; // 禁用精靈圖

  for (const plane of airplanes) {
    // 飛鳥沒有尾跡效果（跳過 contrail 繪製）
    ...
    // 飛鳥的輕微影子（比飛機影子小很多）
    const shadowOpacity = 0.15 * plane.altitude;
...
function drawFallbackAirplane(
  ...
  // 根據高度縮放
  const altitudeScale = 0.5 + plane.altitude * 0.4;
  ctx.scale(altitudeScale, altitudeScale);

  // 翅膀拍動動畫（使用 plane.id 作為相位偏移，讓每隻鳥的翅膀不同步）
  const wingFlap = Math.sin(Date.now() * 0.008 + plane.id * 1.5) * 0.3;
  
  // 飛鳥顏色（根據 plane.color 選擇不同種類的鳥）
  const birdColors: Record<string, { body: string; wing: string }> = {
    '#ffffff': { body: '#f5f5f5', wing: '#e0e0e0' },      // 白鷺
    '#1e40af': { body: '#1a365d', wing: '#2c5282' },      // 蒼鷺
    '#dc2626': { body: '#c53030', wing: '#e53e3e' },      // 朱雀
    '#059669': { body: '#276749', wing: '#38a169' },      // 翠鳥
    '#7c3aed': { body: '#553c9a', wing: '#6b46c1' },      // 紫燕
  };
  
  const colors = birdColors[plane.color] || { body: '#374151', wing: '#4b5563' };
  ...
  ctx.beginPath();
  ctx.moveTo(3, 0);
  ctx.quadraticCurveTo(-2, -8, -8, -12);
  ctx.quadraticCurveTo(-4, -6, -3, 0);
  ctx.closePath();
  ctx.fill();
  ctx.restore();
