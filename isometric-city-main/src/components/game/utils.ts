...
export function screenToGrid(screenX: number, screenY: number, offsetX: number, offsetY: number, zoom: number = 1): { gridX: number; gridY: number } {
  // 1. 先將屏幕座標轉換為相對於地圖原點的縮放後座標（世界座標）
  // worldPos = (screenPos - offset) / zoom
  const worldX = (screenX - offsetX) / zoom;
  const worldY = (screenY - offsetY) / zoom;

  // 2. 減去半個瓦片的偏移量（因為網格繪製是從瓦片頂點開始的）
  const adjustedX = worldX - TILE_WIDTH / 2;
  const adjustedY = worldY - TILE_HEIGHT / 2;
  
  const gridX = (adjustedX / (TILE_WIDTH / 2) + adjustedY / (TILE_HEIGHT / 2)) / 2;
  const gridY = (adjustedY / (TILE_HEIGHT / 2) - adjustedX / (TILE_WIDTH / 2)) / 2;
  
  // Use Math.round for accurate tile selection
