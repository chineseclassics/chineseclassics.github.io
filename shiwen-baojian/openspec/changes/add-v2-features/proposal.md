...
- **批注歷史**：追蹤批注的修改歷史
- **協作修訂**：支持學生基於批注進行論文修訂
- **修訂流程**：學生提交→老師批注→學生撤回修改→重新提交
- **批注錨定**：智能文本錨定，處理學生修改後的批注重新定位
- **統一體驗**：學生端和老師端使用相同的 Google Docs 風格批注界面
- **權限控制**：學生只能編輯自己創建的批注，不能修改老師的批注
...
- 支持協作修訂流程：學生提交→老師批注→學生撤回修改→重新提交
- 批注錨定系統能處理學生修改文本後的批注重新定位
...
### ⏳ 階段 2 進行中
- **精細批注系統**：部分完成（老師端 Google Docs 風格批注已實現）
- **統一批注系統**：進行中（學生端與老師端一致化）
- **協作修訂流程**：待實現（學生提交→老師批注→學生撤回修改→重新提交）
- **批注錨定系統**：待實現（智能文本錨定和重新定位）
- **論文內容統一**：已完成（兩端使用相同的 content_json 格式）
- **即時通知系統**：待實現
...
**提案創建時間**：2025-10-22  
**提案狀態**：階段 1 已完成，階段 2 進行中，階段 3 待開始  
**實際工作量**：69 個任務（原 40-50 個 + 協作修訂流程 6 個 + 統一批注系統 9 個 + 其他 4 個）  
**實際完成時間**：階段 1 約 3 小時，階段 2 進行中

### 詳細任務分解

**階段 2.5 統一批注系統**（9 個任務）：
- 2.5.1 增強 AnnotationManager 支持角色區分（添加權限控制）
- 2.5.2 修改學生端 HTML 佈局（統一容器結構 + 分層顯示模式）
- 2.5.3 處理容器差異（確保 AnnotationManager 在兩端都能找到正確容器）
- 2.5.4 修改學生端初始化邏輯（使用 AnnotationManager）
- 2.5.5 統一 CSS 樣式（移除冗餘樣式）
- 2.5.6 更新批注加載邏輯（統一數據查詢方式）
- 2.5.7 刪除冗餘文件（student-annotation-viewer.js）
- 2.5.8 實現雨村評點收合/展開功能
...
### 統一批注系統（新增）
基於架構優化需求，新增學生端與老師端批注系統統一：

**核心目標**：
- 學生端和老師端使用完全相同的 Google Docs 風格批注界面
- 統一使用 `annotation-manager.js` 作為共享組件
- 實現權限控制：學生只能編輯自己創建的批注
- 消除代碼重複，提高維護性

**技術實現**：
- 增強 AnnotationManager 支持角色區分（teacher/student）
...
- 實現右側側邊欄的分層顯示模式（雨村評點 + 老師批注）
  - 雨村評點區域：可收合設計，默認最小化（只顯示標題和圖標）
  - 老師批注區域：智能跟隨原文高亮，佔據主要顯示空間
  - 動態高度分配：根據內容有無調整顯示比例
- 添加雨村評點的收合/展開功能
  - 默認狀態：雨村評點區域最小化，不影響批注顯示
  - 點擊展開：顯示 AI 反饋內容，可再次點擊收合
  - 收合時：為老師批注留出更多顯示空間
- 實現智能空間分配邏輯
  - 無批注時：雨村評點可佔滿右側空間
  - 有批注時：老師批注優先顯示，雨村評點自動收合
  - 兩者都有時：動態調整高度比例，確保批注跟隨原文高亮
- 處理容器差異，確保 AnnotationManager 在兩端都能找到正確容器
  - 修改 `createFloatingAnnotation` 方法中的容器查找邏輯
  - 支持多種容器選擇器：`.grading-content-wrapper`、`.essay-editor-container`、`#essayViewer` 父元素
- 統一 CSS 樣式，移除冗餘的側邊欄風格
  - 移除舊的側邊欄風格樣式（Line 1-132）
  - 保留對話框樣式（回覆對話框、創建對話框）
  - 保留孤立批注通知樣式
  - 學生端將直接使用 `css/assignment-management.css` 中的 Google Docs 風格樣式
- 刪除冗餘的 `student-annotation-viewer.js` 文件
...
**已實現的統一**：
- **數據格式統一**：學生端和老師端都使用相同的 `content_json` 格式
- **內容顯示統一**：老師端 `GradingUI.renderEssayContent()` 解析 `content_json` 顯示論文
- **字數計算統一**：兩端都使用相同的 `content_json` 解析邏輯計算字數
- **數據結構一致**：兩端看到的論文內容完全相同

**結論**：不需要統一論文渲染器，因為內容統一已經實現，強行統一反而增加複雜性。

### 新增協作修訂流程
基於用戶需求分析，新增論文修訂流程支持：

**核心流程**：
1. **學生提交論文** → 狀態：`submitted`
2. **老師添加批註** → 論文仍為 `submitted`，學生可查看批註
3. **學生撤回修改** → 狀態：`draft`，批註保留
4. **學生基於批註修改** → 批註智能重新定位
5. **學生重新提交** → 狀態：`submitted`，覆蓋原版本
6. **老師最終批改** → 狀態：`graded`，鎖定論文
