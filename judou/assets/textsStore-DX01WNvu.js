import{B as S,u as A,r as d,c as B}from"./index-BekX_NAr.js";import{u as q}from"./useSupabase-WHp6ihkn.js";const C=/[。！？；，：]/g,R=/[「」『』“”"']/g,U=/\|{2,}/g;function w(r){if(!r)return"";const a=r.replace(R,"").replace(C,"|").replace(U,"|").trim();return a.endsWith("|")?a.slice(0,-1):a}function h(r){const s=r.replace(/\|/g,"").length;return s<=70?1:s<=150?2:3}function g(r){return r.replace(/\|/g,"").length}const M=S("texts",()=>{const r=q(),s=A(),i=d([]),a=d(!1),l=d(null),p=B(()=>[...i.value].sort((e,t)=>(t.created_at??"").localeCompare(e.created_at??"")));async function x(){if(!r){l.value="Supabase 尚未配置";return}a.value=!0,l.value=null;try{const{data:e,error:t}=await r.from("practice_texts").select(`
          *,
