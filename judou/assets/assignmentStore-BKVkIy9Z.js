import{x as F,u as P,r as m,I as n}from"./index-mRk65oNV.js";const z=F("assignment",()=>{const l=P(),c=m([]),f=m([]),g=m(new Map),i=m(!1),s=m(null);async function A(e){if(n){i.value=!0,s.value=null;try{const{data:t,error:a}=await n.from("class_assignments").select(`
          *,
          text:practice_texts (
            id,
            title,
        ...
        `).single();return o?(s.value=o.message,null):(c.value.unshift(r),r)}catch(r){return s.value=r.message,null}finally{i.value=!1}}async function x(e){if(!n||!l.isTeacher)return s.value="無權限",!1;i.value=!0,s.value=null;try{const{error:t}=await n.from("class_assignments").delete().eq("id",e);return t?(s.value=t.message,!1):(c.value=c.value.filter(a=>a.id!==e),!0)}catch(t){return s.value=t.message,!1}finally{i.value=!1}}async function q(e){if(n){i.value=!0,s.value=null;try{const{data:t,error:a}=await n.from("assignment_completions").select(`
          *,
