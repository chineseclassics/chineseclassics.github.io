...
- **選擇詞庫模式**（二選一）：
  - **模式 A：使用系統預設詞庫**
    - 從管理員維護的系統詞庫中選擇（如「二年級上冊 - 第一單元」）
    - 系統自動從詞庫中隨機選詞
  - **模式 B：自定義詞語**（像 skribbl.io）
    - 房主手動輸入本輪遊戲的所有詞語
    - 每輪開始時，畫家從這些詞語中選擇一個
- 設置遊戲參數：
  - 每輪繪畫時間（60-180秒）
...
**完全自定義模式**（像 skribbl.io）：
- 房主創建房間時選擇「自定義詞語」模式
- 手動輸入本輪遊戲的所有詞語（一行一個，或逗號分隔）
- 每輪開始時，畫家從這些詞語中隨機選擇一個
- 適合：自由遊戲、朋友聚會、臨時創意
...
**使用流程**：
1. 管理員提前創建系統詞庫（本單元學過的概念）
2. 課堂上，老師或學生創建房間，選擇該系統詞庫
3. 其他學生加入房間（輸入房間碼）
4. 開始遊戲，輪流畫畫和猜詞
5. 遊戲結束後，可以查看統計（哪些詞容易猜、哪些難）
...
**使用流程**：
1. 房主創建房間，選擇「自定義詞語」模式
2. 輸入想要玩的詞語（可以是任何詞語、短語、詩句）
3. 朋友加入房間，開始遊戲
4. 輪流畫畫和猜詞
...
CREATE TABLE game_rooms (
  ...
  wordlist_id UUID REFERENCES wordlists(id),  -- NULL 表示使用自定義詞語
  custom_words TEXT[],  -- 自定義詞語列表（當 wordlist_id 為 NULL 時使用）
  word_mode TEXT DEFAULT 'wordlist',  -- 'wordlist' | 'custom'
...
CREATE TABLE game_rounds (
  ...
  word_text TEXT,  -- 詞語文本（自定義詞語時使用，或作為備份）
