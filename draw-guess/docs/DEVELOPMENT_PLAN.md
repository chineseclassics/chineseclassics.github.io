...
- **構建詞語列表**（混合模式）：
  - **從系統詞庫選擇**：
    ...
    - 可以搜索和篩選詞庫
    - 顯示已選詞語數量
  - **自定義輸入**（參考 skribbl.io 的 Custom words 區域）：
    - 在「Custom words」輸入區即時輸入其他詞語
    - 支持多種輸入格式：
      - 逗號分隔：`春天,友誼,勇氣`
      - 換行分隔：每行一個詞語
      - 混合格式：逗號和換行都可以
    - **可選選項**：「Use custom words only」（僅使用自定義詞語，忽略系統詞庫選擇）
    - 實時驗證和提示
  - **詞語列表預覽**：
    - 顯示已選中的所有詞語（來自詞庫 + 自定義）
    - 標記詞語來源（系統詞庫 / 自定義）
...
- **可選選項**：
  - 「Use custom words only」：勾選後，僅使用自定義詞語，忽略系統詞庫選擇
  - 默認不勾選，使用混合模式
- **實時驗證**：
  - 顯示當前詞語數量
  - 顯示字符數統計
  - 驗證提示（如「還需要 X 個詞語」）
- 與系統詞庫選擇的詞語混合在一起（除非勾選「Use custom words only」）
...
{
  "room_id": "xxx",
  "use_custom_only": false,  // 是否僅使用自定義詞語
  "words": [
    {
      "text": "春天",
...
CREATE TABLE game_rooms (
  ...
  use_custom_only BOOLEAN DEFAULT FALSE,  -- 是否僅使用自定義詞語
...
- [ ] 房主功能（創建房間時）
  ...
  - [ ] 詞語選擇器
    ...
    - [ ] 已選詞語標記和取消選擇
  - [ ] 搜索和篩選功能
  - [ ] 自定義詞語輸入區（參考 skribbl.io 設計）
    - [ ] 大文本區域（支持逗號分隔和換行）
    - [ ] 「Use custom words only」複選框
    - [ ] 實時驗證（字符數、詞語數統計）
    - [ ] 輸入提示和錯誤提示
  - [ ] 詞語列表預覽和管理
...
- 房主可以即時輸入自定義詞語（支持逗號分隔和換行）
- 系統詞庫選擇和自定義詞語可以混合使用（或選擇僅使用自定義）
- 自動去重功能正常工作
- 詞語數量驗證（最少 6 個）正常工作
- 詞語列表正確保存到房間
...
- **Custom words 區域**（白色背景）：
  - 標題：「Custom words」
  - 複選框：「Use custom words only」
  - 大文本輸入區域（多行）
  - 說明文字：
    - 最少 6 個詞語
    - 每個詞語 1-32 個字符
